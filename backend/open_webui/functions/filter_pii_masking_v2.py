...
# 2026-03-20 v2 : 차량번호 마스킹 추가
#   - 한국 차량번호판 마스킹 (신형: 123가1234, 구형: 12가1234, 지역포함: 서울12가1234)
#   - MASK_LICENSE_PLATE Valve 추가 (기본 ON)
############################################
...
class Filter:
    ...
    class Valves(BaseModel):
        ...
        MASK_LICENSE_PLATE: bool = Field(
            default=True,
            description="차량번호판 마스킹 (123가1234, 서울12가1234)"
        )
        MASK_IP: bool = Field(
            default=False,
    ...
    def _get_patterns(valves) -> list[tuple[str, re.Pattern, str]]:
        ...
        # 7) 차량번호판: 한국 번호판 규칙
        #    신형(2019~): 123가1234 → 123가**** / 123가 1234 → 123가 ****
        #    구형:        12가1234  → 12가****  / 12가 1234  → 12가 ****
        #    지역포함:    서울12가1234 → 서울12가****
        if valves.MASK_LICENSE_PLATE:
            # 지역명 포함 구형: 서울12가1234, 경기34나 5678
            patterns.append((
                "차량번호",
                re.compile(
                ...
                lambda m: re.sub(r'\d{4}$', '****', m.group(0))
            ))
            # 신형 3자리+한글+4자리: 123가1234, 123가 1234
            patterns.append((
                "차량번호",
                re.compile(
                    NB_L + r'\d{3}[가-힣]\s?\d{4}' + NB_R
                ),
                lambda m: re.sub(r'\d{4}$', '****', m.group(0))
            ))
            # 구형 2자리+한글+4자리: 12가1234, 12가 1234
            patterns.append((
                "차량번호",
                re.compile(
                    NB_L + r'\d{2}[가-힣]\s?\d{4}' + NB_R
                ),
                lambda m: re.sub(r'\d{4}$', '****', m.group(0))
            ))

        # 8) 여권번호: 알파벳 1-2자 + 7-8자리 (번호 순서 변경됨)
        #    M12345678 → M********
        ...
        # 10) IP 주소 (기본 OFF - 코드/설정에서 자주 쓰이므로)
