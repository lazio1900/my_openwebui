...
# 이력
# 2025-04-16 : 최초 생성
# 2025-04-16 : PDF 규정에서 FAQ 엑셀 데이터로 전환
# 2025-04-04 V2 : retrieval 수정
# 2025-04-04 V21 : 마크다운 처리
# 2025-04-09 V3 : 규정에서 FAQ 데이터로 전환
# 2025-04-16 V4 : pipeline에서 pipe로 전환
# 2025-04-21 V41 : pipeline에서 pipe로 전환 v2
# 2025-04-22 V5 : 디버깅 완료로 다시 pipe로 전환
# 2025-04-23 V6 : 데이터를 wiki로 변경
# 2025-05-02 V7 : wiki 데이터 중 이미지 데이터 출처로 표시
# 2025-05-20 V8 : wiki 데이터 중 웹규정집 데이터 추가 적용
# 2025-05-22 V81 : 웹규정집 제외 결정으로 코드 수정
# 2025-05-29 V9 : FAQ 참고 > 규정 참고 > 일반모델 사용 순으로 변경
# 2025-06-09 V10 : 멀티턴, 라우터, 재질의 개발
# 2025-08-11 V11 : 프롬프트 수정
############################################
...
class Pipe:
    class Valves(BaseModel):
        ...
        BASE_IMG_URL: str = "https://ai.wooricap.com/static/auto_oper_image/"
        STATIC_IMG_URL: str = (
    ...
    def __init__(self):
        ...
        # wiki faq 파싱 및 저장
        documents = self.proc_wiki_faq(self.valves.STATIC_IMG_URL)
        self.save_to_chroma(documents, self.valves.FAQ_COLLECTION_NAME)
        ...
        rule = self.proc_wiki_rule(self.valves.RULE_COLLECTION_ID, filter_docs)
        rule_documents = self.multiple_convert_to_documents(rule)
        self.save_to_chroma(rule_documents, self.valves.RULE_COLLECTION_NAME)

        embedding_func = SentenceTransformerEmbeddings(
            model_name=self.valves.EMBED_PATH, model_kwargs={"device": self.device}
        )

        faq_chroma_db = Chroma(
            client=self.chroma_client,
            collection_name=self.valves.FAQ_COLLECTION_NAME,
            embedding_function=embedding_func,
        )
        rule_chroma_db = Chroma(
            client=self.chroma_client,
        ...
        self.faq_retriever = faq_chroma_db.as_retriever(
            search_kwargs={"k": embedding_k}
        )
        self.rule_retriever = rule_chroma_db.as_retriever(
            search_kwargs={"k": embedding_k}
        )
    ...
    async def pipe(
        ...
        try:
            ...
            documents = self.faq_retrieve(user_message)
            filtered_docs, next_answer = await self.grade_documents(
                documents=documents, user_message=user_message, FLAG="FAQ"
            )
            if next_answer == "아니오":
                generation = await self.generate(
                    documents=filtered_docs, user_message=user_message, history=history
                )
                # yield "디버깅용 FAQ 결과 생성 : \n" + generation
            else:
                documents = self.rule_retrieve(user_message)
                filtered_docs, next_answer = await self.grade_documents(
                    documents=documents, user_message=user_message, FLAG="규정"
                )
                if next_answer == "아니오":
                    generation = await self.generate(
                    ...
                    generation = await self.common_generate(
    ...
    def proc_wiki_faq(self, STATIC_IMG_URL):
        # 이미지 저장 폴더 생성
        ...
        response_data = self.list_docs(self.valves.COLLECTION_ID)
    ...
    async def common_generate(self, user_message, history):
        ...
        # 테스트 데이터 생성
        system = """
        지금 너에게 물어보는 경우에는 회사의 FAQ, 규정을 사용한 RAG에서 답변을 못한
        일반적인 문의에 대해서 물어보는 상황이야.
        업무 자료의 범위를 벗어난 질문이기 때문에 지금 답변은 업무에 활용하기 어렵다고 대답하고,
        질문에 대해서 대답해줘.
        사용자 질문에 대해서 너는 친절하고 간략하고 정확하게 답변해.
        답변 작성시 가독성을 위해 문장이 끝나면 줄바꿈 처리해줘.
        인사 생략해.
        너는 반드시 "한국어"로 답변해.               
        """
    ...
    async def generate(self, documents, user_message, history):

        # 테스트 데이터 생성
        system = """
        너는 JB우리캐피탈 오토운영팀 챗봇이야.
        오토운영팀의 업무는 론, 할부, 중고리스, 전략금융으로 나뉘어져 있어.

        <지침>
        1. 제공되는 참고자료를 참고하여 적합한 답변을 생성하고 DocumentID 같은 IT 단어는 사용하지마
        2. 사용자 질문과 참고자료가 관련성이 떨어진다고 판단하는 경우, 질문과 상황에 대한 상황과 답변의 한계성에 대해서 작성해줘.
            (ex. 질문이 어느 영역 Nego 기준인지는 모르겠지만, 중고리스 Nego기준에 대해서 알려줄게.)
       
        <이미지 url 생성>
        1. 참고자료에 image 태그 포함시, 반드시 "마크다운"으로 변형하여 답변에 마크다운 이미지 URL을 생성해. 
        2. URL이 여러개 있으면 모두 URL을 생성해서 노출해
        ...
        3. 너는 반드시 "한국어"로 답변해.
        4. 이미지 태그가 있는 경우 마크다운 url을 먼저 생성하면서 답변 시작해.
            "URL양식" : ![이미지](https://ai.wooricap.com/static/auto_oper_image/<doc_id>_<image_id>.png)
        5. 이미지가 없는 경우 URL을 생성하지마. 다시 얘기한다. "이미지가 없는 경우 URL을 생성하지마"
