...
# 2026-03-03 v7(운영) : 검색 품질 및 응답 UX 전면 개선
...
#   - _generate(): streaming → non-streaming 전환, 전체 결과 한번에 표시
#   - _generate_with_image(): 동일하게 non-streaming 전환
#   - 단계별 상태 메시지: "답변 생성 중..." → "추천 질문 / 답변 신뢰도 확인 중..." → "완료: X.XX초"
#   - "완료" 시간을 전체 처리 완료 시점으로 변경 (정확한 소요시간 반영)
#   - StreamingResponse/body_iterator 인터셉트 제거 → sync generator(타이핑 효과)
#   - 신뢰도 사유(reason) 텍스트 제거, 점수만 표시
#   - suffix 멘트 제거 ("다른 운영기준이 필요한 경우..." 삭제)
# 2026-03-03 v8(개발) : FAQ 통합 (운영기준 + FAQ 단일 에이전트)
#   - Valves: FAQ_COLLECTION_NAME, ENABLE_FAQ_SEARCH, FAQ_EMBEDDING_K 추가
#   - __init__: FAQ 컬렉션(faq_auto_oper) ChromaDB 초기화 (실패 시 graceful degradation)
#   - _execute_faq_search(): FAQ 컬렉션 벡터 검색 메서드 추가 (subtask 없이 전체 검색)
#   - REACT_STEP_PROMPT_WITH_FAQ: 2개 도구(standard_search, faq_search) 프롬프트 추가
#   - _react_step(): FAQ 활성화 여부에 따라 프롬프트 자동 선택
#   - _parse_react_decision(): tool 필드 파싱 추가 (standard/faq 분기)
#   - _react_agent(): tool 타입별 검색 분기 + 상태 메시지 표시
#   - ENABLE_FAQ_SEARCH=False 시 기존 v7과 동일 동작 (하위 호환)
############################################
...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        # FAQ 검색 설정
        FAQ_COLLECTION_NAME: str = "faq_auto_oper"   # FAQ 컬렉션명
        ENABLE_FAQ_SEARCH: bool = True                # False로 하면 FAQ 검색 비활성화
        FAQ_EMBEDDING_K: int = 3                      # FAQ 검색 시 반환 문서 수
    ...
    def __init__(self):
        ...
        if self.valves.ENABLE_FAQ_SEARCH:
            try:
                ...
                self.valves.ENABLE_FAQ_SEARCH = False
        else:
            self.chroma_db_faq = None

        print("Orchestrator + ReAct 에이전트 Pipe 등록 완료! (v8 운영환경 - FAQ 통합)")
    ...
    async def _react_agent(self, state: dict) -> dict:
        ...
        for i in range(self.valves.REACT_MAX_ITERATIONS):
            ...
            if decision["action"] == "FINISH":
                ...
                tool_type = decision.get("tool", "standard")
                subtask = decision.get("subtask", state["subtask"])

                if tool_type == "faq":
                    display_query = f"{query[:30]}..." if len(query) > 30 else query
                    await send_status(
                        status_message=f"FAQ 검색 중: \"{display_query}\"",
                        done=False,
                    )
                    raw_docs = self._execute_faq_search(query)
                else:
                    display_query = f"{query[:30]}..." if len(query) > 30 else query
                    await send_status(
                        status_message=f"{subtask} 검색 중: \"{display_query}\"",
                        done=False,
                    )
                ...
                source_label = "FAQ" if tool_type == "faq" else subtask
                offset = sum(len(obs["docs"]) for obs in observations)

                observations.append({
                    "query": query,
                    "subtask": source_label,
                    "docs": docs,
                    "doc_summaries": [
    ...
    async def _react_step(self, state: dict, observations: list) -> dict:
        ...
        # FAQ 활성화 여부에 따라 프롬프트 선택
        if self.valves.ENABLE_FAQ_SEARCH and self.chroma_db_faq:
            prompt_template = REACT_STEP_PROMPT_WITH_FAQ
        else:
            prompt_template = REACT_STEP_PROMPT
    ...
    # =================================================================
    # _execute_faq_search() - FAQ 컬렉션 벡터 검색
    # ---------------------------------------------------------------
    # FAQ 컬렉션은 subtask 메타데이터가 없으므로 벡터 검색만 사용.
    # chroma_db_faq가 None이면 빈 리스트 반환 (graceful degradation).
    # =================================================================
    def _execute_faq_search(self, query: str) -> list:
        """FAQ 컬렉션에서 벡터 검색"""
    ...
    # =================================================================
    # _generate() - 최종 답변 생성 (non-streaming, 전체 한번에 표시)
    # ---------------------------------------------------------------
    # ReAct 에이전트가 수집한 문서(filtered_docs)를 참고자료로 활용하여
    # 사용자 질문에 대한 최종 답변을 생성.
    # 상태 메시지로 진행 상황을 표시하고, 완료 후 전체 결과를 한번에 노출.
    #
    # 출력 구조:
    #   헤더 + LLM 답변 + 추천질문 + 신뢰도 → 타이핑 효과로 한번에 표시
    #
    ...
    async def _generate(self, state: dict):
        ...
        # sync generator: 문자 단위 yield (타이핑 효과)
        def stream_output():
    ...
    async def _generate_with_image(self, state: dict):
        """첫 진입시 이미지 + 답변 생성 (non-streaming, 전체 한번에 표시)"""
        domain = state["domain"]
        subtask = state["subtask"]
        filtered_docs = state["filtered_docs"]
        user_message = state["user_message"]
    ...
    def _parse_react_decision(self, content: str, state: dict) -> dict:
        ...
        try:
            ...
            if action == "SEARCH":
                ...
                return {"action": "SEARCH", "tool": tool_type, "query": query, "subtask": subtask}
...
# ==========================================================================
# REACT_STEP_PROMPT_WITH_FAQ - FAQ 도구 포함 버전
# --------------------------------------------------------------------------
# ENABLE_FAQ_SEARCH=True일 때 사용. standard_search + faq_search 2개 도구 제공.
# 입력 변수: {domain}, {subtask}, {available_subtasks}, {observations}, {question}
# ==========================================================================
REACT_STEP_PROMPT_WITH_FAQ = """당신은 JB우리캐피탈 오토운영팀의 정보 검색 에이전트입니다.
사용자 질문에 답하기 위해 검색 도구를 사용하여 관련 문서를 수집합니다.
...
### 도구
1. **standard_search**(query, subtask): **운영기준** 지식베이스에서 관련 문서를 검색합니다.
   - query: 검색어 (한국어, 핵심 키워드 중심)
   - subtask: 위 서브태스크 중 하나
   - 정형화된 규정, 금리표, 네고기준, 운영 규칙 등에 적합

2. **faq_search**(query): **FAQ(자주묻는질문)** 지식베이스에서 관련 문서를 검색합니다.
   - query: 검색어 (한국어, 핵심 키워드 중심)
   - 실무 사례, 업무 처리 방법, 자주 묻는 질문, 구체적 상황 문의에 적합
   - subtask 구분 없이 전체 FAQ에서 검색
...
### 지침
1. 이전 검색 결과가 없으면, 사용자 질문에 가장 적합한 검색어와 도구로 검색하세요.
   - 기본적으로 분류된 서브태스크를 사용하되, 질문 내용에 따라 다른 서브태스크가 적합하면 변경 가능합니다.
2. 이전 검색 결과가 있으면, 결과를 검토하고:
   - 질문에 답하기에 **충분한 정보**가 있으면 → FINISH를 선택하고 관련 문서 번호를 지정하세요.
   - 정보가 **부족**하거나 **다른 관점**의 검색이 필요하면 → 다른 검색어나 도구로 추가 검색하세요.
3. 검색어는 사용자 질문의 핵심 키워드를 추출하여 구체적으로 작성하세요.
4. 동일한 검색어와 도구로 반복 검색하지 마세요.
5. **도구 선택 기준**:
   - 금리, 네고, 한도, 등급, 규정 등 정형화된 기준 → standard_search
   - "어떻게 처리하나요", "이런 경우에는", 업무 절차, FAQ 성격 → faq_search
   - 확신이 없으면 standard_search를 먼저 수행하고, 결과가 부족하면 faq_search를 추가로 수행

### 출력 형식 (JSON만 출력, 다른 텍스트 없이)
운영기준 검색:
{{"action": "SEARCH", "tool": "standard", "thought": "검색 이유", "query": "검색어", "subtask": "(서브태스크명)"}}

FAQ 검색:
{{"action": "SEARCH", "tool": "faq", "thought": "검색 이유", "query": "검색어"}}
