...
#
# 2026-03-23 v7 : 웹규정집 단일 코드 라우팅 + 규정코드별 독립 컬렉션 + 풀네임 헤더
#   - v6 문제(다중 코드 → 문서 오염) 해결: LLM이 반드시 단일 코드 하나만 반환
#   - RULE_CLASSIFY_PROMPT: 복수 코드 반환 금지, rule_code 단수 키로 변경
#   - _parse_rule_classification(): rule_code 단수 파싱 (복수 폴백 제거)
#   - ChromaDB 컬렉션 3개 분리 (벡터 파이프 20260323 연동):
#       chroma_db_rule(단일) → chroma_db_rule_by_code: Dict[str, Chroma]
#       rule_auto_oper_e09003 / rule_auto_oper_e09012 / rule_auto_oper_e09024
#   - BM25 인덱스: 규정코드별 컬렉션에서 직접 로드 (title 그룹핑 방식 제거)
#   - _rule_execute_tool(): post-filtering 완전 제거, 코드별 컬렉션 직접 검색
#   - _rule_react_agent(): chroma_db_rule → chroma_db_rule_by_code 존재 체크
#   - 헤더 풀네임 표시: short_name → display_name
#     (예: "중고승용" → "중고차승용심사운영기준")
#   - GENERAL 코드: 특정 상품 불명확 시 전체 컬렉션 대상 검색
#
# ──────────────────────────────────────────
# 아키텍처 요약
# ──────────────────────────────────────────
...
#   사용자 질문
#       ↓
#   [0] _classify_rule()     : 웹규정집 해당 여부 판단 (LLM 1회) → NONE이면 스킵
#       ↓
#   [1] _rule_react_agent() : 웹규정집 ReAct 검색 (LLM 1~2회, 전체 통합 하이브리드 검색)
#       ↓
...
class Pipe:
    ...
    def __init__(self):
        ...
        # ── 웹규정집: 규정코드별 독립 컬렉션 초기화 ──
        self.chroma_db_rule_by_code: Dict[str, Any] = {}  # Dict[rule_code, Chroma]
        ...
        if self.valves.ENABLE_RULE_SEARCH:
            # ── 코드별 Chroma 컬렉션 연결 ──
            for code in RULE_DOMAIN_MAP.keys():
                col_name = f"{self.valves.RULE_COLLECTION_NAME}_{code.lower()}"
                try:
                    self.chroma_db_rule_by_code[code] = Chroma(
                        client=chroma_client,
                        collection_name=col_name,
                        embedding_function=embedding_func,
                    )
                    print(f"웹규정집 컬렉션 초기화 완료: {col_name}")
                except Exception as e:
                    print(f"웹규정집 컬렉션 초기화 실패 [{code}] ({col_name}): {e}")
            ...
            if self.valves.ENABLE_RULE_HYBRID_SEARCH:
                for code in RULE_DOMAIN_MAP.keys():
                    col_name = f"{self.valves.RULE_COLLECTION_NAME}_{code.lower()}"
                    try:
                        raw_col = chroma_client.get_collection(col_name)
                        data = raw_col.get(include=["documents", "metadatas"])
                        docs = [
                            Document(page_content=t, metadata=m)
                            for t, m in zip(data["documents"], data["metadatas"])
                            if m.get("chpater_merge_yn") != "Y"
                        ]
                        if docs:
                            self._rule_bm25_indices[code] = BM25Okapi([d.page_content.split() for d in docs])
                            self._rule_bm25_docs_by_code[code] = docs
                            print(f"웹규정집 BM25 인덱스 구축 완료 [{code}]: {len(docs)}건")
                    except Exception as e:
                        print(f"웹규정집 BM25 구축 실패 [{code}]: {e}")
    ...
    async def pipe(
        ...
        try:
            ...
            if self.valves.ENABLE_RULE_SEARCH and self.chroma_db_rule_by_code:
    ...
    async def _classify_rule(self, state: dict) -> dict:
        ...
        await send_status(status_message="웹규정집 분류 중...", done=False)
        ...
        code = result["rule_codes"][0] if result["rule_codes"] else "GENERAL"
        if code == "NONE":
            await send_status(status_message="웹규정집 해당 없음 → FAQ 검색으로 전환", done=True)
            print(f"[규정집] NONE - {result.get('thought', '')}")
        elif code == "GENERAL":
            await send_status(status_message="규정 분류: 전체 검색", done=True)
            print(f"[규정집] GENERAL(전체) - {result.get('thought', '')}")
        else:
            display = RULE_DOMAIN_MAP[code]["display_name"]
            await send_status(status_message=f"규정 분류: {display}", done=True)
            print(f"[규정집] {code} ({display}) - {result.get('thought', '')}")
    ...
    def _parse_rule_classification(self, content: str) -> dict:
        try:
            ...
            code = str(data.get("rule_code", "GENERAL")).strip()
            if code not in RULE_DOMAIN_MAP and code not in ("NONE", "GENERAL"):
                code = "GENERAL"
            return {"rule_codes": [code], "thought": data.get("thought", "")}
    ...
    async def _rule_react_agent(self, state: dict) -> dict:
        ...
        if not self.chroma_db_rule_by_code:
            state["rule_docs"] = []
    ...
    def _rule_execute_tool(self, query: str, rule_codes: Optional[List[str]] = None) -> list:
        ...
        # 유효한 규정 코드만 필터링 (GENERAL/NONE 제외) → 없으면 전체 코드 대상
        valid_codes = [c for c in (rule_codes or []) if c in self.chroma_db_rule_by_code]
        target_codes = valid_codes if valid_codes else list(self.chroma_db_rule_by_code.keys())
        ...
        # 벡터 검색: 코드별 컬렉션에서 직접 검색 (post-filtering 불필요)
        all_vector = []
        for code in target_codes:
            db = self.chroma_db_rule_by_code.get(code)
            if db:
                all_vector.extend(db.similarity_search(query=query, k=search_k))
        ...
        for code in target_codes:
            if code not in self._rule_bm25_indices:
                continue
            scores = self._rule_bm25_indices[code].get_scores(tokenized_query)
            for idx, sc in enumerate(scores):
                if sc > 0:
                    all_scored.append((self._rule_bm25_docs_by_code[code][idx], sc))
        all_scored.sort(key=lambda x: x[1], reverse=True)
        bm25_results = [doc for doc, _ in all_scored[:search_k]]

        return self._rule_rrf_merge(all_vector, bm25_results, k)
    ...
    async def _generate(self, state: dict):
        ...
        if answer_source == "rule":
            rule_codes = state.get("rule_codes", [])
            names = [RULE_DOMAIN_MAP[c]["display_name"] for c in rule_codes if c in RULE_DOMAIN_MAP]
            header = f"웹규정집 > {' / '.join(names)}" if names else "웹규정집"
            combined = f"#### [{header}]\n{llm_text}"
        elif answer_source == "faq":
            combined = f"#### [FAQ]\n{llm_text}"
        else:
...
질문에 상품명이 명시되어 있으면 반드시 해당 규정 코드를 선택하세요.
- 중고 승용차 구입/대출/할부 → E09003
- 재고금융/운영자금 → E09012
- 중고 오토리스/리스 → E09024

### 분류 규칙
1. **반드시 코드 하나만 반환** - 복수 코드 반환 절대 금지
2. **멀티턴 유지 (최우선 규칙)**: 이전 규정 코드가 존재하면,
   **다른 규정명(상품명)을 명시적으로 언급**하지 않는 한 이전 코드를 유지한다.
   - "한도", "금리", "조건", "서류", "기준", "비율" 등 일반 용어만으로는 규정 전환 불가
   - 규정 전환은 상품명이 직접 언급될 때만 ("중고리스", "재고금융", "운영자금", "승용" 등)
3. 이전 규정 코드가 없을 때만 질문 내용으로 신규 분류
4. 여러 규정에 해당하더라도 가장 관련성 높은 하나만 반환
5. **GENERAL**: 규정 관련이지만 특정 상품 불명확 → 전체 검색
6. **NONE**: 웹규정집 대상 아님 (실무 FAQ, 업무 절차, 운영기준 등)
...
### 출력 형식 (JSON만 출력, rule_code는 반드시 단수)
{{"rule_code": "E09003", "thought": "분류 근거"}}
해당 없음: {{"rule_code": "NONE", "thought": "분류 근거"}}
불명확: {{"rule_code": "GENERAL", "thought": "분류 근거"}}"""
