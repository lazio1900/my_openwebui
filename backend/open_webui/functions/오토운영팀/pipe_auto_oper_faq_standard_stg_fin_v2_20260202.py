...
class Pipe:
    ...
    def __init__(self):
        ...
        self.retriever1 = stand_chroma_db_text.as_retriever(search_kwargs={"filter": {"subtask": "(재고금융)"}, "k": embedding_k})
    ...
    async def pipe(
        ...
        try:
            ...
            await send_status(status_message=f"분류 결과 : 전략금융 > {classified}", done = True)
            
            
            if classified == "재질의":
                generation = await self.common_generate(user_message = user_message, history=history)
                return generation
            elif classified == '재고금융':
                documents = self.retriever1.invoke(user_message)
                filtered_docs, next_answer = await self.grade_documents(documents=documents, user_message=user_message, FLAG="전략금융 > 재고금융")
                ...
                filtered_docs, next_answer = await self.grade_documents(documents=documents, user_message=user_message, FLAG="전략금융 > 제휴점 운영자금")
                generation = await self.generate(documents = filtered_docs, user_message = user_message, history=history, classified = classified, last_classified = last_classified)
                await self._update_state(user_id, classified)
                return generation
            elif classified == '매매상사 운영자금':
                documents = self.retriever3.invoke(user_message)
                filtered_docs, next_answer = await self.grade_documents(documents=documents, user_message=user_message, FLAG="전략금융 > 매매상사 운영자금")
                generation = await self.generate(documents = filtered_docs, user_message = user_message, history=history, classified = classified, last_classified = last_classified)
                await self._update_state(user_id, classified)
                return generation
            elif classified == '운영자금 자금용도 기준':
                documents = self.retriever4.invoke(user_message)
                filtered_docs, next_answer = await self.grade_documents(documents=documents, user_message=user_message, FLAG="전략금융 > 운영자금 자금용도 기준")
                generation = await self.generate(documents = filtered_docs, user_message = user_message, history=history, classified = classified, last_classified = last_classified)
                await self._update_state(user_id, classified)
                return generation       
            elif classified == '임차보증금':
                documents = self.retriever5.invoke(user_message)
                filtered_docs, next_answer = await self.grade_documents(documents=documents, user_message=user_message, FLAG="전략금융 > 임차보증금")
                generation = await self.generate(documents = filtered_docs, user_message = user_message, history=history, classified = classified, last_classified = last_classified)
                await self._update_state(user_id, classified)
                return generation    
    ...
    async def common_generate(self, user_message, history):
        ...
        # 테스트 데이터 생성
        system = """
        지금 너에게 물어보는 경우에는 '재고금융', '제휴점 운영자금', '매매상사 운영자금', '운영자금 자금용도 기준', '임차보증금' 업무 분류에 벗어난 질문을 한 상태야
        재질문으로 사용자에게 다시 입력을 유도해.
        보통은 "내용에서 벗어난 질문입니다. 전략금융 업무 중 '재고금융', '제휴점 운영자금', '매매상사 운영자금', '운영자금 자금용도 기준', '임차보증금' 중 문의해주세요." 정도로 답하고
        가능하면 사용자 질문이나 그간 대화내용을 참고해서 더 나은 질문에 대해서 답변해줘. 
        예를들어 "운영기준 알려줘"와 같은 너무 넓은 범위 질문이 들어오면,
        추천질문으로 "재고금융 운영기준 알려줘", '제휴점 운영자금 운영기준 알려줘' 와 같은 추천질문을 제공함으로써, 사용자가 업무 영역으로 넘어가도록 추천질문을 제공해줘.
        너는 반드시 "한국어"로 답변해.               
        """
    ...
    async def generate(self, documents, user_message, history, classified, last_classified):
        ...
        너가 대답하는 경우는 오토운영팀 전략금융 대출 상품에 대해 상세 문의 하는 경우야
        큰 업무 분류 기준은 '재고금융', '제휴점 운영자금', '매매상사 운영자금', '운영자금 자금용도 기준', '임차보증금' 이며, 
    ...
    async def requery_and_classify_task(self, question):
        ...
        class TotalTask(BaseModel):
            classifed_task: str = Field(
                description="사용자 질문에 대한 업무 분류. ('재고금융', '제휴점 운영자금', '매매상사 운영자금', '운영자금 자금용도 기준', '임차보증금', '재질의')"
            )
        ...
        [분류 기준]
        분류 기준은 '재고금융', '제휴점 운영자금', '매매상사 운영자금', '운영자금 자금용도 기준', '임차보증금', '재질의' 이며 5개 업무와 1개 업무를 벗어난 '재질의'로 분류하시오.
        사용자는 특정 업무가 분류된 이후로 업무에 관련된 상세 질문을 추가할 수 있습니다.(상품명, 대상, 금리, 프로모션, 판촉수수료, 슬라이딩, 연체이자율, 중도상환, 실적 등) 이점 참고하세요
        
        <재고금융>
        정의 : 재고금융은 중고차 매매상사를 대상으로 차량 재고를 담보로 대출·한도·이자·LTV 등을 제공하는 금융상품으로, 약정·실사·한도산정·등급관리·연체·사후관리를 포괄하는 운영기준이 적용된다.
        관련 질의 : 재고금융 한도 기준은 어떻게 설정돼?, 재고금융 약정서 작성시 필요한 서류는?, 매매상사 등급화 점수표와 적용 기준은?, 연체 발생 시 사후관리 절차는?

        <제휴점 운영자금>
        정의 : 제휴점 운영자금은 제휴점(협력 점포)의 월 매출 규모에 따라 평가된 한도 내에서 12~60개월 기간, 연 5.8∼6.3% 금리로 제공되는 운용 대출 상품이며, 매출 대비 대출 원금이 40%를 초과하면 추가 대출이 제한되고, 분기 실적에 따라 목표 달성 시 금리 복원, 미달성 시 최대 15% 이내 금리 인상(패널티)이 적용된다.
        관련 질의 : 제휴점 운영자금 한도는 어떻게 산정돼?, 운영자금 대출 금리와 기간은 어떻게 정해져?, 목표 달성 여부에 따른 패널티 적용 기준은?, 연체 시 제휴점에 어떤 사후관리 조치가 이루어져?

        <매매상사 운영자금>
        정의 : 매매상사 운영자금은 최근 3개월 최고 판매월의 판매대수와 NICE CB 점수에 따라 한도가 결정되는 대출상품으로, 12 ~ 60개월 기간에 5.8 % ~ 6.3 % 금리(신용등급 1~4 등급 기준)로 제공되며, 재고금융·운영자금 합산 한도 30 억원 이하일 경우 영업본부장 전결이 필요하고, 연체·Fraud 등 사후관리 절차가 별도로 운영된다.
        관련 질의 : 매매상사 운영자금 한도는 판매대수와 NICE CB 점수에 따라 어떻게 산정돼?

        <운영자금 자금용도 기준>
        정의 : 운영자금 자금용도 기준은 대출금 사용 목적별로 요구되는 증빙서류·증빙방법·미이행 시 적용되는 Penalty(기한이익상실·대출금 회수 등)를 명시한 규정이며, 타사대환, 임차보증금, 인테리어, 기타(상기외) 등으로 구분하고, 법인·개인별 금액 기준(예: 법인 1건 5억 초과·총합 10억 초과)·주택 소유권 이전 시 담보취득 여부 등에 따라 별도 제한을 두고 있다.
        관련 질의 : 운영자금 타사대환 시 요구되는 증빙서류와 제출 기한은?

        <임차보증금>
        정의 : 재고금융은 기업이 보유하고 있는 재고(상품·부품·원자재 등)를 담보로 하여 자금을 조달하는 금융상품으로, 재고를 매각·보관·유통하는 과정에서 발생하는 현금 흐름을 안정적으로 지원한다.
        관련 질의 : “임차보증금 운영자금 금리는 몇 %인가요?”

        <재질의>
        정의 : '재고금융', '제휴점 운영자금', '매매상사 운영자금', '운영자금 자금용도 기준', '임차보증금' 외 상품 문의(업무관련 공통 질의 포함) 또는 상품 중 모호한 질의인 경우 재질의 실시
        관련 질의 : 전략금융 상품에 대해서 알려줘 또는 운영기준 알려줘
        예상 답변 : '재고금융', '제휴점 운영자금', '매매상사 운영자금', '운영자금 자금용도 기준', '임차보증금'에 대해서 문의해주세요. 또는 '재고금융', '제휴점 운영자금', '매매상사 운영자금', '운영자금 자금용도 기준', '임차보증금' 중 어떤 업무에 관한 문의인가요?
            
        
        다음 JSON 스키마를 정확히 준수하여 한국어로만 응답하시오.
        {{
        "classifed_task": "재고금융|제휴점 운영자금|매매상사 운영자금|운영자금 자금용도 기준|임차보증금|재질의"
        }}
