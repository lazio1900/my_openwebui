...
# 2026-03-20 : FAQ + 웹규정집 수집 통합
#            : kure 임베딩 통일 (기존 koe5 → kure)
#            : FAQ Wiki Outline API 수집 추가 (faq_auto_oper 컬렉션)
#            : 웹규정집 Wiki Outline API 수집 추가 (rule_auto_oper 컬렉션)
# 2026-03-23 : 웹규정집 규정코드별 컬렉션 분리 저장
#            : 단일 컬렉션(rule_auto_oper) → 규정코드별 3개 컬렉션 분리
#              rule_auto_oper_e09003 / rule_auto_oper_e09012 / rule_auto_oper_e09024
#            : title 앞 6자(규정코드) 기준 그룹핑 후 코드별 save_to_chroma() 호출
#            : pipe_auto_oper_chat_agent_v7.py 와 컬렉션 구조 동기화
############################################
...
class Pipe:
    class Valves(BaseModel):
        ...
        EMBED_PATH: str = "/data1/embedding/kure"  # kure 임베딩 통일
    ...
    def __init__(self):
        ...
        self.CHATBOT_FUNCTION_IDS = [
            ...
            "auto_oper_chat_agent",  # v6 통합 에이전트 (웹규정집+FAQ+운영기준)
        ]

 

    def _connect_chroma(self, max_retries=3):
        """ChromaDB 연결 - 재시도 + heartbeat 체크로 안전하게 연결"""
        for attempt in range(max_retries):
            try:
                client = chromadb.HttpClient(
                    host=self.valves.CHROMA_IP,
                    port=self.valves.CHROMA_PORT,
                )
                client.heartbeat()  # 실제 연결 확인
                return client
            except Exception as e:
                print(f"[vector_db] ChromaDB 연결 시도 {attempt+1}/{max_retries} 실패: {e}")
    ...
    def _invalidate_chatbot_cache(self):
        """
        app.state에서 챗봇 함수 캐시를 무효화하여 다음 호출 시 재초기화되도록 합니다.
        벡터 DB 업데이트 후 호출하여 챗봇이 최신 데이터로 동작하게 합니다.
        Returns: 무효화된 함수 ID 리스트
        """
        invalidated = []
        try:
            ...
            for cid in self.CHATBOT_FUNCTION_IDS:
                ...
                if cleared:
                    ...
                    print(f"[vector_db] '{cid}' 캐시에 없음 (이미 초기 상태)")

            print(f"[vector_db] 챗봇 함수 무효화 완료: {invalidated}")
        except Exception as e:
    ...
    def pipe(
        ...
        try:
            ...
            # ── 1단계: ChromaDB 연결 (재시도 포함) ──
            self.chroma_client = self._connect_chroma(max_retries=3)
            if self.chroma_client is None:
                yield "\n❌ ChromaDB 연결 실패 (3회 재시도 후 포기) - 작업을 중단합니다.\n"
                yield "💡 ChromaDB 서버 상태를 확인해주세요: " \
                      f"{self.valves.CHROMA_IP}:{self.valves.CHROMA_PORT}\n"
                return
            yield f"\n✅ chroma_client 객체 생성 (heartbeat 확인 완료)\n"
            ...
            # ── 5단계: FAQ Wiki 수집 + 저장 ──
            try:
                faq_docs = self.proc_faq_wiki()
                yield f"\n✅ FAQ 문서 수집 완료: {len(faq_docs)}건\n"

                self.save_to_chroma(faq_docs, self.valves.FAQ_COLLECTION_NAME)
                yield f"\n✅ FAQ docs 저장 완료 ({self.valves.FAQ_COLLECTION_NAME})\n"
            except Exception as e:
                yield f"\n❌ FAQ 처리 실패: {e}\n"

            # ── 6단계: 웹규정집 Wiki 수집 + 규정코드별 컬렉션 분리 저장 ──
            try:
                rule_docs = self.proc_rule_wiki()
                yield f"\n✅ 웹규정집 문서 수집 완료: {len(rule_docs)}건\n"

                rule_groups: dict = {}
                for doc in rule_docs:
                    m = re.search(r'([A-Z]\d{5})', doc.metadata.get("title", ""))
                    code = m.group(1) if m else "UNKNOWN"
                    rule_groups.setdefault(code, []).append(doc)

                for code, docs in rule_groups.items():
                    col_name = f"{self.valves.RULE_COLLECTION_NAME}_{code.lower()}"
                    self.save_to_chroma(docs, col_name)
                    yield f"\n✅ 웹규정집 [{code}] {len(docs)}건 저장 완료 ({col_name})\n"
            except Exception as e:
                yield f"\n❌ 웹규정집 처리 실패: {e}\n"
            ...
            yield f"\n✅ 전체 처리 완료({exe_time:.2f})초\n"

            # ── 7단계: 연관 챗봇 Pipe 캐시 무효화 (재기동) ──
            invalidated = self._invalidate_chatbot_cache()
            yield f"\n✅ 챗봇 함수 캐시 무효화 완료: {invalidated}\n"
    ...
    def proc_faq_wiki(self):
        """Outline wiki에서 FAQ 문서 다운로드 + 이미지 처리 → Document 리스트"""
        os.makedirs(self.valves.FAQ_STATIC_IMG_PATH, exist_ok=True)
        self.delete_all_files_in_directory(self.valves.FAQ_STATIC_IMG_PATH)

        response_data = self.list_docs(self.valves.FAQ_WIKI_COLLECTION_ID)
        ...
        for data in response_data:
            ...
            urls = self._faq_extract_image_urls(text)
            html_text = markdown("**질문** : \n" + title + "\n\n" + text)

            modified_text = text
            for idx, url in enumerate(urls):
                image_id = self._faq_extract_id(url)
                file_name = self._faq_download_image(url, doc_id, image_id)

                modified_text = self._faq_replace_image_url_to_tag(modified_text, file_name)
                modified_text = self._faq_process_content_with_images(modified_text)
                html_text = markdown("**질문** : \n" + title + "\n\n" + modified_text)
    ...
    def _faq_extract_image_urls(self, markdown_text):
    ...
    def _faq_extract_id(self, url):
        """URL에서 이미지 ID 추출"""
        parsed_url = urlparse(url)
        query_params = parse_qs(parsed_url.query)
        return query_params.get("id", [None])[0]

    def _faq_download_image(self, url, document_id, image_id):
        """이미지 다운로드 + 로컬 저장"""
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {self.valves.OUTLINE_API_KEY}",
    ...
    def _faq_replace_image_url_to_tag(self, text, file_name):
    ...
    def _faq_process_content_with_images(self, content):
    ...
    def proc_rule_wiki(self):
        ...
        for doc_info in raw_docs:
            ...
            structure = self._rule_make_structure(text)
            chapters, etc_sections = self._rule_classify_sections(structure)
            all_documents.extend(self._rule_process_chapters(chapters, title, source))
            all_documents.extend(self._rule_process_etc(etc_sections, title, source))

        return all_documents

    def _rule_make_structure(self, all_text):
    ...
    def _rule_classify_sections(self, structure):
    ...
    def _rule_process_chapters(self, structure, title, source):
    ...
    def _rule_process_etc(self, structure, title, source):
