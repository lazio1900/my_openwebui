...
#
# 2026-03-05 v2 : FAQ 추천질문 제거
#   - FAQ 답변 시 추천질문(후속질문) 생성 스킵 (LLM 1회 절약)
#   - 운영기준 답변에만 추천질문 유지
#   - 신뢰도(grounding check)는 FAQ에도 유지
#
# 2026-03-10 v3 : FAQ 검색 품질 개선
#   - FAQ 하이브리드 검색: 벡터+BM25+RRF 적용 (기존 벡터 only → 하이브리드)
#   - FAQ 충분성 판단 프롬프트 고도화: 5가지 세분화 기준 + 신뢰도 점수
#   - FAQ BM25 인덱스 init 시 구축 (ENABLE_FAQ_HYBRID_SEARCH Valve)
#
# 2026-03-10 v4 : FAQ ReAct 에이전트 + 전체 이미지 노출
#   - FAQ에도 ReAct RAG 적용: _faq_react_agent() 추가 (Think→Act→Observe)
#   - FAQ 충분성 판단 제거 → FAQ ReAct가 자체적으로 검색 충분성 판단
#   - 흐름 변경: FAQ ReAct → 충분하면 faq 답변 / 불충분하면 classify → 운영기준 ReAct
#   - 전체 도메인 has_image=True (중고승용 외 전략금융/중고리스/중형트럭도 이미지 노출)
#   - FAQ_REACT_MAX_ITERATIONS Valve 추가
#
# 2026-03-18 v5 : 웹규정집(Rule) ReAct 에이전트 추가 - 3계층 폴백
#   - 웹규정집 ReAct 에이전트 추가: _rule_react_agent() (Think→Act→Observe)
#   - 3계층 폴백: 웹규정집 → FAQ → 운영기준 순으로 검색
#   - 웹규정집 wiki sync: Outline API → 제X장/제X절/제X조 구조 파싱 → ChromaDB 저장
#   - 웹규정집 하이브리드 검색: 벡터+BM25+RRF (FAQ/운영기준과 동일 기술 스택)
#   - ENABLE_RULE_SEARCH=False 시 v4 동일 동작 (하위 호환)
#   - 규정 citation: 초록색 테마 HTML 카드 + 원문 링크
#   - 규정 답변 헤더: "#### [웹규정집]"
#
# 2026-03-20 v6 : 웹규정집 상품별 라우터 도입 (운영기준 방식 적용)
#   - LLM 기반 규정 분류: _classify_rule() 추가 (키워드 감지 → LLM 분류로 전환)
#   - 상품별 BM25 인덱스 분리: 규정코드별(E09003/E09012/E09024) 독립 인덱스
#   - RULE_DOMAIN_MAP: 규정코드 → 상품 정보 매핑
#   - 웹규정집 헤더 상품명 표시: "#### [웹규정집 > 중고승용]"
#   - NONE 분류 시 웹규정집 스킵 (FAQ/운영기준으로 직행, LLM 절약)
#   - 멀티턴 규정 코드 유지: last_rule_codes 상태 관리 (복수 코드 지원)
#   - ENABLE_RULE_CLASSIFY=False 시 기존 키워드 방식 폴백
#
# ──────────────────────────────────────────
# 아키텍처 요약
# ──────────────────────────────────────────
# [흐름] 웹규정집 ReAct → FAQ ReAct → 운영기준 ReAct (3계층 폴백)
#
#   사용자 질문
#       ↓
#   [0] _classify_rule()     : 웹규정집 상품 분류 (LLM 1회) → NONE이면 스킵
#       ↓
#   [1] _rule_react_agent() : 웹규정집 ReAct 검색 (LLM 1~2회, 상품별 하이브리드 검색)
#       ↓
#       ├─ 규정 충분 → _generate(rule) → 종료    ← classify 스킵
#       ↓
#   [2] _faq_react_agent()  : FAQ ReAct 검색 (LLM 1~2회, 하이브리드 검색)
#       ↓
#       ├─ FAQ 충분 → _generate(faq) → 종료      ← classify 스킵
#       ↓
...
#   _rule_react_agent()     : 웹규정집 ReAct 루프 (Think→Act→Observe, 규정 컬렉션 검색)
#   _rule_react_step()      : 웹규정집 ReAct 단일 스텝 LLM 호출
#   _rule_execute_tool()    : 웹규정집 하이브리드 검색 (벡터+BM25+RRF)
#   _rule_rrf_merge()       : 웹규정집 벡터+BM25 RRF 결합
#   _faq_react_agent()      : FAQ ReAct 루프 (Think→Act→Observe, FAQ 컬렉션 검색)
#   _faq_react_step()       : FAQ ReAct 단일 스텝 LLM 호출
#   _faq_execute_tool()     : FAQ 하이브리드 검색 (벡터+BM25+RRF)
#   _classify()             : 도메인/서브태스크 분류 + 재질의 감지
#   _react_agent()          : 운영기준 ReAct 루프 (Think→Act→Observe, 최대 3회)
#   _react_step()           : 운영기준 ReAct 단일 스텝 LLM 호출
#   _execute_tool()         : 운영기준 하이브리드 검색 (벡터+BM25+RRF)
#   _rrf_merge()            : 벡터+BM25 하이브리드 검색 RRF 결합
#   _generate()             : 최종 답변 생성 (answer_source별 프롬프트 분기)
#   _generate_with_image()  : 이미지 포함 답변
#   _generate_followups()   : 후속질문 생성 (병렬)
#   _check_grounding()      : 답변 신뢰도 검증 (병렬)
#   _format_citation_html() : 출처를 HTML 카드로 포맷팅
#   _requery()              : 재질의 응답
#   _proc_wiki_rule()       : Outline wiki → 웹규정집 Document 리스트
#   _proc_wiki_faq()        : Outline wiki → FAQ Document 리스트
#   _save_to_chroma()       : Document → ChromaDB 저장
#
# [출처 표시]
#   규정 : "#### [웹규정집 > 상품명]"   (예: 웹규정집 > 중고승용)
#   FAQ  : "#### [FAQ]"              (서브태스크 없음)
#   표준 : "#### [도메인 > 서브태스크]"  (예: 중고승용 운영기준 > (론/할부))
#
# [citation 포맷]
#   규정 : HTML 카드 (초록색 테마, markdown→HTML 변환 + 원문 링크)
#   FAQ  : HTML 카드 (Q. 질문 박스 + 답변 + 원문 링크)
#   표준 : page_content를 markdown→HTML 변환
#
# [소스 파일]
#   v7.py     : 운영기준 챗봇 (Orchestrator + ReAct)
#   FAQ v11   : FAQ 챗봇 (wiki sync, 이미지 처리, 웹규정집 파싱)
...
##########################################################################
# 웹규정집 도메인 매핑 (규정코드 → 상품 정보)
##########################################################################
RULE_DOMAIN_MAP = {
    "E09003": {"display_name": "중고차승용심사운영기준", "short_name": "중고승용"},
    "E09012": {"display_name": "재고금융/운영자금심사운영기준", "short_name": "재고금융/운영자금"},
    "E09024": {"display_name": "중고오토리스심사운영기준", "short_name": "중고리스"},
}
...
class Pipe:
    class Valves(BaseModel):
        ...
        ENABLE_FAQ_WIKI_SYNC: bool = False  # 벡터 파이프에서 처리하므로 기본 비활성화
        ENABLE_FAQ_HYBRID_SEARCH: bool = True
        FAQ_COLLECTION_NAME: str = "faq_auto_oper"
        FAQ_EMBEDDING_K: int = 3
        FAQ_REACT_MAX_ITERATIONS: int = 2
        FAQ_RRF_K: int = 60
        FAQ_RRF_VECTOR_WEIGHT: float = 0.7
        FAQ_RRF_BM25_WEIGHT: float = 0.3
        ...
        # ── 웹규정집 설정 ──
        ENABLE_RULE_SEARCH: bool = True
        ENABLE_RULE_CLASSIFY: bool = True  # LLM 기반 규정 분류 (False 시 기존 키워드 방식)
        ENABLE_RULE_WIKI_SYNC: bool = False
        ENABLE_RULE_HYBRID_SEARCH: bool = True
        RULE_COLLECTION_NAME: str = "rule_auto_oper"
        RULE_EMBEDDING_K: int = 5
        RULE_REACT_MAX_ITERATIONS: int = 2
        RULE_RRF_K: int = 60
        RULE_RRF_VECTOR_WEIGHT: float = 0.7
        RULE_RRF_BM25_WEIGHT: float = 0.3
        RULE_WIKI_COLLECTION_ID: str = "c6472f73-b9af-48a3-945e-8a16320f4ef6"
        RULE_FILTER_DOCS: str = "E09003,E09012,E09024"
        # ── 웹규정집 상품 키워드 매핑 (유사 규정 구분용) ──
        # JSON 형식: {"키워드": "규정코드", ...}
        RULE_PRODUCT_KEYWORDS: str = '{"중고승용":"E09003","중고차승용":"E09003","승용차":"E09003","승용":"E09003","중고차":"E09003","재고금융":"E09012","운영자금":"E09012","중고리스":"E09024","오토리스":"E09024","중고오토리스":"E09024"}'
    ...
    def __init__(self):
        ...
        if self.valves.ENABLE_FAQ_SEARCH:
            ...
            # ── FAQ BM25 인덱스 구축 (하이브리드 검색용) ──
            self._faq_bm25_index = None
            self._faq_bm25_docs = []

            if self.valves.ENABLE_FAQ_HYBRID_SEARCH:
                try:
                    raw_faq_collection = chroma_client.get_collection(
                        self.valves.FAQ_COLLECTION_NAME
                    )
                    faq_all_data = raw_faq_collection.get(include=["documents", "metadatas"])

                    self._faq_bm25_docs = [
                        Document(page_content=doc_text, metadata=meta)
                        for doc_text, meta in zip(faq_all_data["documents"], faq_all_data["metadatas"])
                    ]

                    if self._faq_bm25_docs:
                        tokenized_corpus = [d.page_content.split() for d in self._faq_bm25_docs]
                        self._faq_bm25_index = BM25Okapi(tokenized_corpus)
                        print(f"FAQ BM25 인덱스 구축 완료: {len(self._faq_bm25_docs)}건 문서")
                except Exception as e:
                    print(f"FAQ BM25 인덱스 구축 실패 (벡터 검색만 사용): {e}")

        # ── ChromaDB 클라이언트 보관 (save_to_chroma에서 사용) ──
        self._chroma_client = chroma_client

        # ── 웹규정집: wiki sync + 컬렉션 초기화 ──
        self.chroma_db_rule = None
        self._rule_bm25_indices = {}   # Dict[rule_code, BM25Okapi]
        self._rule_bm25_docs_by_code = {}  # Dict[rule_code, List[Document]]

        if self.valves.ENABLE_RULE_SEARCH:
            # ChromaDB 네이티브 임베딩 함수 (upsert용)
            if not hasattr(self, '_faq_chroma_ef'):
                self._faq_chroma_ef = embedding_functions.SentenceTransformerEmbeddingFunction(
                    model_name=self.valves.EMBED_PATH, device="cpu"
                )

            if self.valves.ENABLE_RULE_WIKI_SYNC:
                try:
                    rule_documents = self._proc_wiki_rule()
                    self._save_to_chroma(rule_documents, self.valves.RULE_COLLECTION_NAME)
                    print(f"웹규정집 wiki sync 완료: {len(rule_documents)}건 문서")
                except Exception as e:
                    print(f"웹규정집 wiki sync 실패: {e}")

            try:
                self.chroma_db_rule = Chroma(
                    client=chroma_client,
                    collection_name=self.valves.RULE_COLLECTION_NAME,
                    embedding_function=embedding_func,
                )
                print(f"웹규정집 컬렉션 초기화 완료: {self.valves.RULE_COLLECTION_NAME}")
            except Exception as e:
                print(f"웹규정집 컬렉션 초기화 실패 (웹규정집 검색 비활성화): {e}")
                self.chroma_db_rule = None

            # ── 웹규정집 BM25 인덱스 구축 (상품별 분리) ──
            if self.valves.ENABLE_RULE_HYBRID_SEARCH:
                try:
                    raw_rule_collection = chroma_client.get_collection(
                        self.valves.RULE_COLLECTION_NAME
                    )
                    rule_all_data = raw_rule_collection.get(include=["documents", "metadatas"])

                    rule_groups: Dict[str, list] = {}
                    for doc_text, meta in zip(rule_all_data["documents"], rule_all_data["metadatas"]):
                        if meta.get("chpater_merge_yn") == "Y":
                            continue
                        title = meta.get("title", "")
                        code = self._extract_rule_code(title)
                        if code not in rule_groups:
                            rule_groups[code] = []
                        rule_groups[code].append(
                            Document(page_content=doc_text, metadata=meta)
                        )

                    for code, docs in rule_groups.items():
                        tokenized_corpus = [d.page_content.split() for d in docs]
                        self._rule_bm25_indices[code] = BM25Okapi(tokenized_corpus)
                        self._rule_bm25_docs_by_code[code] = docs

                    total_docs = sum(len(d) for d in rule_groups.values())
                    print(f"웹규정집 BM25 인덱스 구축 완료: {len(rule_groups)}개 규정, 총 {total_docs}건 문서")
                except Exception as e:
                    print(f"웹규정집 BM25 인덱스 구축 실패 (벡터 검색만 사용): {e}")

        print("통합 에이전트 Pipe 등록 완료! (v6 - 웹규정집 상품별 라우터 + FAQ + 운영기준 3계층 폴백)")

    # =================================================================
    # pipe() - 메인 엔트리포인트
    # ---------------------------------------------------------------
    # 흐름:
    #   1. _rule_react_agent() : 웹규정집 ReAct 검색 (LLM 1~2회)
    #   1a. 규정 충분 → _generate(rule) : 규정 답변 생성
    #   2. _faq_react_agent() : FAQ ReAct 검색 (LLM 1~2회)
    #   2a. FAQ 충분 → _generate(faq) : FAQ 답변 생성
    #   2b. FAQ 불충분 → _classify() : 도메인/서브태스크 분류
    #   3. _requery()         : 분류 불가 시 재질의
    #   4. _react_agent()     : 운영기준 ReAct 검색 (LLM 1~2회)
    #   5. _generate(standard) : 운영기준 답변 생성
    # =================================================================
    async def pipe(
        self,
        body: dict,
        ...
        try:
            ...
            state = {
                ...
                "last_rule_codes": user_state.get("last_rule_codes", []),
            }

            # ── 1단계: 웹규정집 분류 + ReAct 에이전트 ──
            if self.valves.ENABLE_RULE_SEARCH and self.chroma_db_rule:
                if self.valves.ENABLE_RULE_CLASSIFY:
                    state = await self._classify_rule(state)
                else:
                    # 폴백: 기존 키워드 방식
                    detected = self._detect_rule_product(state["user_message"])
                    state["rule_codes"] = [detected] if detected else ["GENERAL"]

                state = await self._rule_react_agent(state)

            if state["rule_sufficient"]:
                # 웹규정집에서 충분한 문서를 찾음 → classify 스킵, 바로 답변
                state["filtered_docs"] = state["rule_docs"]
                state["answer_source"] = "rule"
                return await self._generate(state)

            # ── 2단계: FAQ ReAct 에이전트 ──
            if self.valves.ENABLE_FAQ_SEARCH and self.chroma_db_faq:
                state = await self._faq_react_agent(state)

            if state["faq_sufficient"]:
                # FAQ ReAct가 충분한 문서를 찾음 → classify 스킵, 바로 답변
                state["filtered_docs"] = state["faq_docs"]
                state["answer_source"] = "faq"
                return await self._generate(state)

            # ── 3단계: FAQ 불충분 → 운영기준 검색 ──
            if self.valves.ENABLE_STANDARD_SEARCH:
    ...
    # =================================================================
    # _rule_react_agent() - 웹규정집 ReAct 루프
    # ---------------------------------------------------------------
    # Think→Act→Observe 패턴으로 웹규정집 컬렉션을 검색.
    # 충분한 문서를 찾으면 rule_sufficient=True, 못 찾으면 False (FAQ 폴백)
    # =================================================================
    def _detect_rule_product(self, question: str) -> Optional[str]:
        """사용자 질문에서 상품 키워드를 감지하여 규정 코드 반환. 미감지 시 None."""
        try:
            keyword_map = json.loads(self.valves.RULE_PRODUCT_KEYWORDS)
        except (json.JSONDecodeError, TypeError):
            return None
        # 긴 키워드 우선 매칭 (예: '중고오토리스'가 '중고'보다 먼저)
    ...
    # =================================================================
    # _classify_rule() - 웹규정집 LLM 기반 상품 분류
    # ---------------------------------------------------------------
    # 질문이 어떤 규정에 해당하는지 LLM으로 분류.
    # 결과: rule_codes (List[str]) - 복수 코드 지원
    # =================================================================
    async def _classify_rule(self, state: dict) -> dict:
        send_status = state["send_status"]
        await send_status(status_message="웹규정집 질문 분류 중...", done=False)

        history_summary = self._build_history_summary(state["messages"])

        last_codes = state.get("last_rule_codes", [])
        last_codes_str = ", ".join(last_codes) if last_codes else "없음"

        prompt = RULE_CLASSIFY_PROMPT.format(
            history=history_summary,
            last_rule_codes=last_codes_str,
            question=state["user_message"],
        )

        content = await self._llm_call(prompt, state)
        result = self._parse_rule_classification(content)

        state["rule_codes"] = result["rule_codes"]

        # 상태 메시지: 규정명 표시
        names = []
        for code in result["rule_codes"]:
            info = RULE_DOMAIN_MAP.get(code)
            if info:
                names.append(info["short_name"])
        if names:
            display = ", ".join(names)
            await send_status(
                status_message=f"규정 분류: {display}",
                done=True,
            )
            print(f"[규정집] LLM 분류: {result['rule_codes']} ({display}) - {result.get('thought', '')}")
        else:
            await send_status(
                status_message=f"규정 분류: {result['rule_codes']}",
                done=True,
            )
            print(f"[규정집] LLM 분류: {result['rule_codes']} - {result.get('thought', '')}")

        return state

    def _parse_rule_classification(self, content: str) -> dict:
        try:
            cleaned = re.sub(r"```(?:json)?", "", content).strip().strip("`")
            data = json.loads(cleaned)
            # 복수 코드 지원: rule_codes (리스트) 우선, rule_code (단일) 하위 호환
            raw = data.get("rule_codes") or data.get("rule_code", "NONE")
            if isinstance(raw, str):
                raw = [raw]
            # 유효한 코드만 필터링
            valid = [c for c in raw if c in RULE_DOMAIN_MAP or c in ("NONE", "GENERAL")]
            rule_codes = valid if valid else ["GENERAL"]
            return {"rule_codes": rule_codes, "thought": data.get("thought", "")}
        except (json.JSONDecodeError, KeyError, AttributeError):
            return {"rule_codes": ["GENERAL"], "thought": "파싱 실패, 전체 검색으로 폴백"}

    @staticmethod
    def _extract_rule_code(title: str) -> str:
        """타이틀에서 규정 코드 추출 (예: 'E09003중고차승용심사운영기준' → 'E09003')"""
        match = re.match(r'^([A-Z]\d{5})', title)
        return match.group(1) if match else title

    async def _rule_react_agent(self, state: dict) -> dict:
        observations = []
        seen_contents = set()
        send_status = state["send_status"]

        if not self.chroma_db_rule:
            state["rule_docs"] = []
            state["rule_sufficient"] = False
            return state

        # 분류된 규정 코드 사용 (NONE이면 웹규정집 스킵)
        rule_codes = state.get("rule_codes", [])
        if rule_codes == ["NONE"]:
            state["rule_docs"] = []
            state["rule_sufficient"] = False
            await send_status(
                status_message="웹규정집 해당 없음 → FAQ 검색으로 전환",
                done=True,
            )
            return state

        for i in range(self.valves.RULE_REACT_MAX_ITERATIONS):
            await send_status(
                status_message=f"웹규정집 ReAct 검색 중... (반복 {i + 1}/{self.valves.RULE_REACT_MAX_ITERATIONS})",
                done=False,
            )

            decision = await self._rule_react_step(state, observations)
            ...
            if decision["action"] == "SUFFICIENT":
                ...
                if relevant_indices and all_docs:
                    ...
                    state["rule_docs"] = all_docs

                if not state["rule_docs"] and all_docs:
                    state["rule_docs"] = all_docs

                state["rule_sufficient"] = True
                await send_status(
                    status_message=f"웹규정집 문서 {len(state['rule_docs'])}건 수집 완료",
                    done=True,
                )
                return state

            elif decision["action"] == "INSUFFICIENT":
                state["rule_docs"] = []
                state["rule_sufficient"] = False
                await send_status(
                    status_message="웹규정집에서 충분한 정보를 찾지 못함 → FAQ 검색으로 전환",
                    done=True,
                )
                return state
                ...
                await send_status(
                    status_message=f"웹규정집 검색 중: \"{query[:30]}...\"" if len(query) > 30 else f"웹규정집 검색 중: \"{query}\"",
                    done=False,
                )

                raw_docs = self._rule_execute_tool(query, rule_codes=rule_codes)
                ...
                observations.append({
                    ...
                    "docs": docs,
                    "doc_summaries": [
                        f"[규정문서{offset + j}] {d.page_content[:200]}"
        ...
        if all_docs:
            ...
            state["rule_sufficient"] = True
            await send_status(
                status_message=f"웹규정집 문서 {len(all_docs)}건 수집 완료 (최대 반복 도달)",
                done=True,
            )
        else:
            state["rule_docs"] = []
    ...
    # =================================================================
    # _rule_react_step() - 웹규정집 ReAct 1스텝 판단
    # =================================================================
    async def _rule_react_step(self, state: dict, observations: list) -> dict:
        if observations:
            obs_text_parts = []
            for idx, obs in enumerate(observations):
                obs_text_parts.append(
                    f"--- 웹규정집 검색 {idx + 1}: query=\"{obs['query']}\" ---"
                )
        ...
        prompt = RULE_REACT_STEP_PROMPT.format(
            observations=obs_text,
            question=state["user_message"],
        )

        content = await self._llm_call(prompt, state)
        return self._parse_faq_react_decision(content)  # 동일 파싱 로직 재사용

    # =================================================================
    # _rule_execute_tool() - 웹규정집 하이브리드 검색 (벡터 + BM25 + RRF)
    # =================================================================
    def _rule_execute_tool(self, query: str, rule_codes: Optional[List[str]] = None) -> list:
        k = self.valves.RULE_EMBEDDING_K

        # 유효한 규정 코드만 필터링 (GENERAL/NONE 제외)
        valid_codes = [c for c in (rule_codes or []) if c in RULE_DOMAIN_MAP]

        use_hybrid = (
            self.valves.ENABLE_RULE_HYBRID_SEARCH
            and len(self._rule_bm25_indices) > 0
        )

        if not use_hybrid:
            results = self.chroma_db_rule.similarity_search(query=query, k=k * 3 if valid_codes else k)
            if valid_codes:
                results = [d for d in results if self._extract_rule_code(d.metadata.get("title", "")) in valid_codes]
            return results[:k]

        search_k = k * 2

        # 벡터 검색 (넓게 가져온 후 코드별 필터링)
        fetch_k = search_k * 3 if valid_codes else search_k
        raw_vector = self.chroma_db_rule.similarity_search(query=query, k=fetch_k)
        if valid_codes:
            vector_results = [d for d in raw_vector if self._extract_rule_code(d.metadata.get("title", "")) in valid_codes][:search_k]
        else:
            vector_results = raw_vector[:search_k]

        # BM25 검색 (상품별 인덱스 사용)
        tokenized_query = query.split()
        # 검색 대상 코드 결정: 유효한 코드가 있으면 해당 코드만, 없으면 전체
        target_codes = valid_codes if valid_codes else list(self._rule_bm25_indices.keys())
        all_scored = []
        for code in target_codes:
            if code not in self._rule_bm25_indices:
                continue
            bm25 = self._rule_bm25_indices[code]
            docs = self._rule_bm25_docs_by_code[code]
            scores = bm25.get_scores(tokenized_query)
            for idx, sc in enumerate(scores):
                if sc > 0:
                    all_scored.append((docs[idx], sc))
        all_scored.sort(key=lambda x: x[1], reverse=True)
        bm25_results = [doc for doc, _ in all_scored[:search_k]]

        return self._rule_rrf_merge(vector_results, bm25_results, k)

    # =================================================================
    # _rule_rrf_merge() - 웹규정집용 Reciprocal Rank Fusion
    # =================================================================
    def _rule_rrf_merge(self, vector_docs: list, bm25_docs: list, k: int) -> list:
        rrf_k = self.valves.RULE_RRF_K
        w_vec = self.valves.RULE_RRF_VECTOR_WEIGHT
        w_bm25 = self.valves.RULE_RRF_BM25_WEIGHT
        doc_scores = {}
        doc_objects = {}
    ...
    # =================================================================
    # _faq_react_agent() - FAQ ReAct 루프
    # ---------------------------------------------------------------
    # Think→Act→Observe 패턴으로 FAQ 컬렉션을 검색.
    # 충분한 문서를 찾으면 faq_sufficient=True, 못 찾으면 False (운영기준 폴백)
    # =================================================================
    async def _faq_react_agent(self, state: dict) -> dict:
        observations = []
        seen_contents = set()
        send_status = state["send_status"]
        ...
        for i in range(self.valves.FAQ_REACT_MAX_ITERATIONS):
            await send_status(
                status_message=f"FAQ ReAct 검색 중... (반복 {i + 1}/{self.valves.FAQ_REACT_MAX_ITERATIONS})",
                done=False,
            )

            decision = await self._faq_react_step(state, observations)
            ...
            if decision["action"] == "SUFFICIENT":
                ...
                if relevant_indices and all_docs:
                    ...
                    state["faq_docs"] = all_docs

                if not state["faq_docs"] and all_docs:
                    state["faq_docs"] = all_docs

                state["faq_sufficient"] = True
                await send_status(
                    status_message=f"FAQ 문서 {len(state['faq_docs'])}건 수집 완료",
                    done=True,
                )
                return state
                ...
                state["faq_sufficient"] = False
                await send_status(
                    status_message="FAQ에서 충분한 정보를 찾지 못함 → 운영기준 검색으로 전환",
                    done=True,
                )
                return state
                ...
                await send_status(
                    status_message=f"FAQ 검색 중: \"{query[:30]}...\"" if len(query) > 30 else f"FAQ 검색 중: \"{query}\"",
                    done=False,
                )

                raw_docs = self._faq_execute_tool(query)
                ...
                observations.append({
                    ...
                    "docs": docs,
                    "doc_summaries": [
                        f"[FAQ문서{offset + j}] {d.page_content[:200]}"
                        for j, d in enumerate(docs)
                    ],
                })

        # 최대 반복 도달 → 수집된 문서가 있으면 사용, 없으면 불충분
        all_docs = []
        for obs in observations:
            all_docs.extend(obs["docs"])
        ...
        if all_docs:
            ...
            state["faq_sufficient"] = True
            await send_status(
                status_message=f"FAQ 문서 {len(all_docs)}건 수집 완료 (최대 반복 도달)",
                done=True,
            )
        else:
            state["faq_docs"] = []
    ...
    # =================================================================
    # _faq_react_step() - FAQ ReAct 1스텝 판단
    # =================================================================
    async def _faq_react_step(self, state: dict, observations: list) -> dict:
        if observations:
            obs_text_parts = []
            for idx, obs in enumerate(observations):
                obs_text_parts.append(
                    f"--- FAQ 검색 {idx + 1}: query=\"{obs['query']}\" ---"
                )
                obs_text_parts.extend(obs["doc_summaries"])
            obs_text = "\n".join(obs_text_parts)
        else:
        ...
        prompt = FAQ_REACT_STEP_PROMPT.format(
            observations=obs_text,
            question=state["user_message"],
        )

        content = await self._llm_call(prompt, state)
        return self._parse_faq_react_decision(content)

    # =================================================================
    # _faq_execute_tool() - FAQ 하이브리드 검색 (벡터 + BM25 + RRF)
    # =================================================================
    def _faq_execute_tool(self, query: str) -> list:
        k = self.valves.FAQ_EMBEDDING_K

        use_hybrid = (
            self.valves.ENABLE_FAQ_HYBRID_SEARCH
            and self._faq_bm25_index is not None
            and len(self._faq_bm25_docs) > 0
        )

        if not use_hybrid:
            return self.chroma_db_faq.similarity_search(query=query, k=k)

        search_k = k * 2

        vector_results = self.chroma_db_faq.similarity_search(query=query, k=search_k)

        tokenized_query = query.split()
        scores = self._faq_bm25_index.get_scores(tokenized_query)
        scored_indices = [
            (idx, sc) for idx, sc in enumerate(scores) if sc > 0
        ]
        scored_indices.sort(key=lambda x: x[1], reverse=True)
        bm25_results = [self._faq_bm25_docs[idx] for idx, _ in scored_indices[:search_k]]

        return self._faq_rrf_merge(vector_results, bm25_results, k)

    # =================================================================
    # _faq_rrf_merge() - FAQ용 Reciprocal Rank Fusion
    # =================================================================
    def _faq_rrf_merge(self, vector_docs: list, bm25_docs: list, k: int) -> list:
        rrf_k = self.valves.FAQ_RRF_K
        w_vec = self.valves.FAQ_RRF_VECTOR_WEIGHT
        w_bm25 = self.valves.FAQ_RRF_BM25_WEIGHT
        doc_scores = {}
        doc_objects = {}
    ...
    # =================================================================
    # _parse_faq_react_decision() - FAQ ReAct 결과 파싱
    # =================================================================
    def _parse_faq_react_decision(self, content: str) -> dict:
        try:
            cleaned = re.sub(r"```(?:json)?", "", content).strip().strip("`")
            data = json.loads(cleaned)
            action = data.get("action", "INSUFFICIENT")
    ...
    def _format_citation_html(self, doc, idx: int, answer_source: str) -> str:
        if answer_source == "rule":
            content_html = markdown(doc.page_content)
            title = doc.metadata.get("title", "")
            chapter = doc.metadata.get("chapter", "")
            source_url = doc.metadata.get("source", "")
            header = f"{title} {chapter}".strip()

            source_link = ""
            if source_url:
                source_link = (
                    '<div style="margin-top:12px;padding-top:8px;border-top:1px solid #e5e7eb;font-size:12px">'
                    f'<a href="{source_url}" target="_blank" style="color:#059669;text-decoration:none">원문 보기</a>'
                    '</div>'
                )

            return (
                '<div style="font-family:-apple-system,sans-serif;font-size:14px;line-height:1.6">'
                '<div style="background:#ecfdf5;padding:10px 14px;border-radius:8px;margin-bottom:10px;border-left:4px solid #059669">'
                f'<strong style="color:#059669">{header}</strong>'
                '</div>'
                f'<div style="padding:0 4px">{content_html}</div>'
                f'{source_link}'
                '</div>'
            )
        elif answer_source == "faq":
    ...
    # =================================================================
    # _generate() - 최종 답변 생성 (웹규정집/FAQ/운영기준 통합)
    # ---------------------------------------------------------------
    # answer_source에 따라 프롬프트 분기:
    #   - "rule": RULE_GENERATE_PROMPT (조문 인용 포함)
    ...
    async def _generate(self, state: dict):
        ...
        for idx, doc in enumerate(filtered_docs, start=1):
            ...
            title_text = doc.metadata.get("title", f"출처{idx}") if answer_source in ("faq", "rule") else f"출처{idx}"
            await send_citation(
                url=f"출처{idx}",
                title=title_text,
        ...
        followup_task = None
        grounding_task = None
        if self.valves.ENABLE_FOLLOWUPS and answer_source not in ("faq", "rule"):
        ...
        if followup_task or grounding_task:
            await send_status(
                status_message="추천 질문 / 답변 신뢰도 확인 중..." if followup_task else "답변 신뢰도 확인 중...",
                done=False,
            )
        ...
        # ── 멀티턴 상태 업데이트 (웹규정집) ──
        if answer_source == "rule" and state.get("rule_codes"):
            await self._update_state(
                state["user_id"], "", "", rule_codes=state["rule_codes"]
            )

        # ── 최종 결과 조합 ──
        if answer_source == "rule":
            rule_codes = state.get("rule_codes", [])
            names = [RULE_DOMAIN_MAP[c]["short_name"] for c in rule_codes if c in RULE_DOMAIN_MAP]
            if names:
                combined = f"#### [웹규정집 > {' / '.join(names)}]\n{llm_text}"
            else:
                combined = f"#### [웹규정집]\n{llm_text}"
        elif answer_source == "faq":
            combined = f"#### [FAQ]\n{llm_text}"
    ...
    async def _update_state(self, user_id: str, domain: str, subtask: str, rule_codes: List[str] = None) -> None:
        async with self._state_lock:
            prev = self._state_by_user.get(user_id, {})
            self._state_by_user[user_id] = {
                "last_classified": subtask,
                "last_domain": domain,
                "last_rule_codes": rule_codes if rule_codes else prev.get("last_rule_codes", []),
                "ts": time.time(),
            }
    ...
    # =================================================================
    # 웹규정집 데이터 처리 메서드 (FAQ v11에서 이식)
    # =================================================================

    def _proc_wiki_rule(self) -> List[Document]:
        """Outline wiki에서 웹규정집 문서 다운로드 + 구조 파싱 → Document 리스트"""
        filter_docs = [
            s.strip() for s in self.valves.RULE_FILTER_DOCS.split(",") if s.strip()
        ]
        collection_id = self.valves.RULE_WIKI_COLLECTION_ID

        response_data = self._list_docs(collection_id)

        raw_docs = []
        for result in response_data:
            # 체크 표시가 있는 문서만 유효
            if result.get("icon") != "\u2714\ufe0f":
                continue
        ...
        for doc_info in raw_docs:
            ...
            structure = self._make_rule_structure(text)
            chapters, etc_sections = self._classify_rule_sections(structure)
            all_documents.extend(self._process_rule_chapters(chapters, title, source))
            all_documents.extend(self._process_rule_etc(etc_sections, title, source))

        return all_documents

    def _make_rule_structure(self, all_text: str) -> list:
        """규정 텍스트를 제X장/제X절/제X조 + 별표/별지서식 구조로 파싱"""
    ...
    def _classify_rule_sections(self, structure: list) -> tuple:
        """구조를 chapters와 etc_sections(별표/별지서식)으로 분류"""
        chapters = []
        etc_sections = []
        for section in structure:
            if "chapter" in section:
                chapters.append(section)
            elif "etc_title" in section:
                etc_sections.append(section)
        return chapters, etc_sections

    def _process_rule_chapters(self, structure: list, title: str, source: str) -> List[Document]:
        """장/절/조 구조를 조별 Document로 변환"""
        documents = []
        chapter_merge = []
        ...
        for chapter in structure:
            ...
            for section in chapter.get("sections", []):
                section_number = section.get("section", "") or ""
    ...
    def _process_rule_etc(self, structure: list, title: str, source: str) -> List[Document]:
        """별표/별지서식을 Document로 변환"""
        documents = []
        for etc in structure:
            etc_content = etc.get("etc_content", "")
            etc_title = etc.get("etc_title", "")
            if etc_title:
                etc_content = "\n".join(etc_content) if isinstance(etc_content, list) else etc_content
...
# ==========================================================================
# RULE_CLASSIFY_PROMPT - 웹규정집 LLM 기반 상품 분류
# ==========================================================================
RULE_CLASSIFY_PROMPT = """당신은 사용자의 질문이 어떤 사내규정(웹규정집)에 해당하는지 분류하는 모델입니다.

### 규정 목록
1. **E09003** - 중고차승용심사운영기준: 중고 승용차 론/할부 심사 기준
   - 키워드: 중고승용, 중고차, 승용차, 론, 할부, 구입대출
2. **E09012** - 재고금융/운영자금심사운영기준: 재고금융, 운영자금 심사 기준
   - 키워드: 재고금융, 운영자금, 매매상사, 약정서, 실사
3. **E09024** - 중고오토리스심사운영기준: 중고 오토리스 심사 기준
   - 키워드: 중고리스, 오토리스, 운용리스, 금융리스, 잔가

### 주의: 유사 규정 구분
E09003, E09012, E09024는 상품만 다르고 구조가 매우 유사합니다.
- 중고 승용차 구입/대출/할부 → E09003
- 재고금융/운영자금 → E09012
- 중고 오토리스/리스 → E09024

### 분류 규칙
1. 사용자 질문의 키워드와 맥락을 분석하여 가장 적합한 규정 코드를 선택
2. **복수 규정 해당**: 질문이 여러 규정에 공통으로 해당할 수 있으면 복수 코드를 반환 (예: 외국인 취급 기준은 E09003, E09024 모두 해당)
3. **멀티턴 유지**: 이전 규정 코드가 있으면, 다른 규정/상품을 명시적으로 언급하지 않는 한 이전 규정 유지
4. **NONE**: 사내규정이 아닌 질문 (실무 FAQ, 운영기준 수준의 상세 정보, 업무 외 질문)
5. **GENERAL**: 특정 규정을 특정할 수 없지만 규정 관련 질문인 경우

### 이전 규정 코드: {last_rule_codes}
...
### 출력 형식 (JSON만 출력)
{{"rule_codes": ["E09003"], "thought": "분류 근거"}}
복수 해당 시: {{"rule_codes": ["E09003", "E09024"], "thought": "분류 근거"}}
해당 없음: {{"rule_codes": ["NONE"], "thought": "분류 근거"}}"""


# ==========================================================================
# RULE_REACT_STEP_PROMPT - 웹규정집 ReAct 1스텝 판단
# ==========================================================================
RULE_REACT_STEP_PROMPT = """당신은 JB우리캐피탈 오토운영팀의 웹규정집(사내규정) 검색 에이전트입니다.
사용자 질문에 답하기 위해 사내규정 지식베이스를 검색하여 관련 규정 조문을 수집합니다.

### 도구
- rule_search(query): 사내규정(웹규정집) 지식베이스에서 관련 조문을 검색합니다.
  - query: 검색어 (사용자 질문의 핵심 키워드, 한국어)

### 주의: 유사 상품 규정 구분
아래 3개 규정은 **상품만 다르고 구조가 유사한 심사운영기준**입니다. 반드시 구분하세요:
- **E09003**: 중고차승용심사운영기준 (중고 승용차 론/할부)
- **E09012**: 재고금융/운영자금심사운영기준
- **E09024**: 중고오토리스심사운영기준 (중고 오토리스)
검색 결과에 여러 상품의 규정이 섞여 있으면, **사용자가 질문한 상품과 관련된 규정만** relevant_doc_indices로 선택하세요.
...
### 지침
1. 이전 검색 결과가 없으면 → 사용자 질문에서 핵심 키워드를 추출하여 SEARCH를 실행하세요.
2. 이전 검색 결과가 있으면, 규정 문서를 검토하고:
   - 규정 조문이 질문에 **직접적으로 관련**되고 **법적/규정적 근거를 제공**하면 → SUFFICIENT
   - 규정 문서가 **부분적으로만 관련**되거나 핵심 조문이 부족하면 → 다른 검색어로 추가 SEARCH
   - 사내규정에서 **답변할 수 없는 질문**이라고 판단되면 → INSUFFICIENT (FAQ 검색으로 넘김)
3. **SUFFICIENT 판단 기준** (아래 조건을 모두 충족해야 함):
   - 규정 조문(제X조)이 사용자 질문의 의도와 직접 관련됨
   - 규정 내용만으로 추가 추측 없이 답변 가능
   - 규정의 정의, 기준, 절차 등이 질문에 대한 명확한 근거를 제공
4. **INSUFFICIENT 판단 기준** (아래 중 하나라도 해당):
   - 규정 문서가 사용자 질문과 관련 없음
   - 실무 FAQ나 운영기준 수준의 구체적 정보가 필요한 질문 (예: 특정 금리, 수수료율)
   - 업무 영역 밖의 질문 (날씨, 인사 등)
5. 동일한 검색어로 반복 검색하지 마세요.

### 출력 형식 (JSON만 출력, 다른 텍스트 없이)
검색이 필요한 경우:
{{"action": "SEARCH", "thought": "검색 이유를 간단히 설명", "query": "검색어"}}

규정으로 충분한 답변이 가능한 경우:
{{"action": "SUFFICIENT", "thought": "충분하다고 판단한 근거", "relevant_doc_indices": [0, 1]}}

규정으로 답변이 불가능하여 FAQ 검색이 필요한 경우:
{{"action": "INSUFFICIENT", "thought": "불충분하다고 판단한 근거"}}

- relevant_doc_indices는 이전 검색 결과의 모든 규정 문서를 0부터 순서대로 번호를 매긴 것입니다.
- 질문과 직접 관련 있는 규정 문서만 선택하세요."""


# ==========================================================================
# RULE_GENERATE_PROMPT - 웹규정집 기반 답변 생성
# ==========================================================================
RULE_GENERATE_PROMPT = """너는 JB우리캐피탈 오토운영팀 챗봇이야.

사용자에게 주어진 질문에 대해서 참고자료(사내규정/웹규정집)를 활용해서 적절히 답변해.

<지침>
1. 제공되는 참고자료(규정 조문)를 참고하여 적합한 답변을 생성하고 DocumentID 같은 IT 단어는 사용하지마
2. 사용자 질문이 너무 광범위할 경우, 구체적으로 알려달라고 요청해.

<조문 인용 원칙>
1. 답변에 관련 규정 조문 번호(예: 제X조)를 명시하여 근거를 밝혀줘.
2. 여러 조문이 관련되면 각각의 조문 번호와 내용을 구분하여 안내해.
3. 규정의 장/절 구조가 있으면 어떤 규정의 몇 장 몇 조인지 명확히 표시해.
...
<유사 상품 규정 구분 원칙 - 반드시 준수>
1. 아래 3개 규정은 **상품만 다르고 구조가 유사**합니다. 반드시 어떤 상품의 규정인지 명확히 구분하여 답변해:
   - 중고차승용심사운영기준(E09003): 중고 승용차 론/할부
   - 재고금융/운영자금심사운영기준(E09012): 재고금융, 운영자금
   - 중고오토리스심사운영기준(E09024): 중고 오토리스
2. 답변 시 **어떤 규정(상품)**의 내용인지 규정명을 명시해.
3. 참고자료에 **여러 상품의 규정이 섞여 있고**, 사용자가 **특정 상품을 지정하지 않은 경우**:
   - "중고승용/재고금융/중고오토리스 중 어떤 상품에 대해 문의하시는 건가요?" 라고 역질문해.
   - 단, 질문 내용이 특정 상품과 명확히 관련되면 해당 상품 기준으로 답변해도 됨.
4. 다른 상품의 규정 내용을 **혼동하여 답변하지 마**.
...
# ==========================================================================
# FAQ_REACT_STEP_PROMPT - FAQ ReAct 1스텝 판단
# ==========================================================================
FAQ_REACT_STEP_PROMPT = """당신은 JB우리캐피탈 오토운영팀의 FAQ 검색 에이전트입니다.
사용자 질문에 답하기 위해 FAQ 지식베이스를 검색하여 관련 문서를 수집합니다.

### 도구
- faq_search(query): FAQ 지식베이스에서 관련 문서를 검색합니다.
  - query: 검색어 (사용자 질문의 핵심 키워드, 한국어)
...
### 지침
1. 이전 검색 결과가 없으면 → 사용자 질문에서 핵심 키워드를 추출하여 SEARCH를 실행하세요.
2. 이전 검색 결과가 있으면, FAQ 문서를 검토하고:
   - FAQ 문서가 질문에 **직접적으로 관련**되고 **완전한 답변이 가능**하면 → SUFFICIENT
   - FAQ 문서가 **부분적으로만 관련**되거나 핵심 정보가 부족하면 → 다른 검색어로 추가 SEARCH
   - FAQ에서 **답변할 수 없는 질문**이라고 판단되면 → INSUFFICIENT (운영기준 검색으로 넘김)
3. **SUFFICIENT 판단 기준** (아래 조건을 모두 충족해야 함):
   - FAQ 문서의 질문(Q)이 사용자 질문의 의도와 일치
   - FAQ 답변(A)만으로 추가 추측 없이 완전한 답변 가능
   - 1~2개의 FAQ 문서로 답변이 완결됨
4. **INSUFFICIENT 판단 기준** (아래 중 하나라도 해당):
   - FAQ 문서가 사용자 질문과 관련 없음
   - 업무 규정/기준/수치 등 운영기준 수준의 상세 정보가 필요한 질문
   - 업무 영역 밖의 질문 (날씨, 인사 등)
5. 동일한 검색어로 반복 검색하지 마세요.
...
FAQ로 답변이 불가능하여 운영기준 검색이 필요한 경우:
{{"action": "INSUFFICIENT", "thought": "불충분하다고 판단한 근거"}}

- relevant_doc_indices는 이전 검색 결과의 모든 FAQ 문서를 0부터 순서대로 번호를 매긴 것입니다.
- 질문과 직접 관련 있는 FAQ 문서만 선택하세요."""
