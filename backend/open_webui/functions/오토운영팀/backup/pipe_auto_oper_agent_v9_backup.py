...
# 2026-03-03 v9(개발) : FAQ 우선 → 운영기준 폴백 아키텍처
#   - FAQ 컬렉션(faq_auto_oper) 벡터 검색 후 LLM 충분성 판단
#   - FAQ 충분 → ReAct 생략, FAQ docs로 바로 답변 생성 (빠른 경로)
#   - FAQ 불충분 → 기존 ReAct 운영기준 검색 (v7 동일)
#   - _faq_search(): FAQ 벡터 검색 메서드 추가
#   - _evaluate_faq(): FAQ 충분성 LLM 판단 메서드 추가
#   - FAQ_EVALUATE_PROMPT: FAQ 적합성 판단 프롬프트 추가
#   - pipe(): FAQ 분기 로직 삽입 (_classify 후, _react_agent 전)
...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        FAQ_EMBEDDING_K: int = 5                      # FAQ 검색 시 반환 문서 수
    ...
    def __init__(self):
        ...
        print("Orchestrator + ReAct 에이전트 Pipe 등록 완료! (v9 운영환경 - FAQ 우선)")
    ...
    async def pipe(
        ...
        try:
            ...
            state = {
                ...
                # FAQ 검색 결과 (v9)
                "faq_docs": [],                   # FAQ 검색 문서 리스트
                "faq_sufficient": False,          # FAQ로 답변 충분 여부
                "answer_source": "standard",      # 답변 소스 ("faq" 또는 "standard")
                # 멀티턴 판단용 이전 상태
    ...
    # =================================================================
    # _faq_search() - FAQ 벡터 검색 (v9)
    # ---------------------------------------------------------------
    # FAQ 컬렉션에서 사용자 질문과 유사한 문서를 검색.
    # subtask 필터 없이 전체 FAQ에서 검색.
    # state["faq_docs"]에 결과 저장.
    # =================================================================
    async def _faq_search(self, state: dict):
        """FAQ 컬렉션 벡터 검색 → state['faq_docs']에 저장"""
        send_status = state["send_status"]

        if not self.valves.ENABLE_FAQ_SEARCH or not self.chroma_db_faq:
            state["faq_docs"] = []
            return state
    ...
    # =================================================================
    # _evaluate_faq() - FAQ 충분성 LLM 판단 (v9)
    # ---------------------------------------------------------------
    # FAQ 검색 결과가 사용자 질문에 충분한지 LLM이 판단.
    # 충분하면 ReAct를 건너뛰고 FAQ docs로 바로 답변 생성.
    # FAQ 결과가 비어있으면 LLM 호출 없이 바로 불충분 판정.
    # =================================================================
    async def _evaluate_faq(self, state: dict):
        """FAQ 검색 결과로 답변 가능 여부를 LLM이 판단 → state['faq_sufficient']에 저장"""
    ...
    async def _generate(self, state: dict):
        ...
        source_prefix = "FAQ" if state.get("answer_source") == "faq" else "출처"
        for idx, doc in enumerate(filtered_docs, start=1):
            await send_citation(
                url=f"{source_prefix}{idx}",
                title=f"{source_prefix}{idx}",
                content=doc.page_content,
            )

        # ── 답변 생성 (non-streaming) ──
        source_label = "FAQ 기반 " if state.get("answer_source") == "faq" else ""
        await send_status(status_message=f"{source_label}답변 생성 중...", done=False)
...
# ==========================================================================
# FAQ_EVALUATE_PROMPT - FAQ 충분성 판단 프롬프트 (v9)
# --------------------------------------------------------------------------
# 입력 변수: {faq_context}, {question}
# 출력: JSON {"sufficient": true/false, "reason": "판단 근거"}
# ==========================================================================
