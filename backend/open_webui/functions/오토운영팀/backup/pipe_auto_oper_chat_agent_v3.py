...
#   사용자 질문
#       ↓
#   [1] _faq_search()      : FAQ 하이브리드 검색 (벡터+BM25+RRF, LLM 호출 없음)
#       ↓
...
#   _faq_search()           : FAQ 하이브리드 검색 (벡터+BM25+RRF, FAQ_EMBEDDING_K개)
...
class Pipe:
    ...
    def __init__(self):
        ...
        print("통합 에이전트 Pipe 등록 완료! (v3 - FAQ 하이브리드 검색 + 충분성 판단 고도화)")
    ...
    # =================================================================
    # _faq_search() - FAQ 하이브리드 검색 (벡터 + BM25 + RRF)
    # =================================================================
    async def _faq_search(self, state: dict) -> dict:
        ...
        try:
            ...
            # 벡터 검색
            vector_results = self.chroma_db_faq.similarity_search(
                query=state["user_message"],
                k=search_k,
            )

            # BM25 검색
            tokenized_query = state["user_message"].split()
            ...
            # RRF 결합
            state["faq_docs"] = self._faq_rrf_merge(vector_results, bm25_results, k)
    ...
    async def _evaluate_faq(self, state: dict) -> dict:
        ...
        try:
            ...
            # confidence 4 이상이면 충분, 3 이하면 불충분 (프롬프트 기준과 이중 검증)
            state["faq_sufficient"] = sufficient and confidence >= 4
            print(f"FAQ 충분성 판단: sufficient={sufficient}, confidence={confidence}/5, reason={data.get('reason', '')}")
...
### 판단 기준 (5가지 항목을 모두 평가)
1. **질문-FAQ 의미 일치도**: 사용자 질문의 핵심 의도와 FAQ 질문(Q)이 동일한 주제를 다루고 있는가?
   - 단순 키워드 일치가 아닌, 질문의 **의도와 맥락**이 일치해야 함
   - 예: "금리 알려줘" → FAQ에 "론/할부 금리표"가 있어도, 어떤 상품 금리인지 특정 불가하면 불충분
2. **답변 완전성**: FAQ 답변(A)만으로 사용자가 원하는 정보를 **빠짐없이** 제공할 수 있는가?
   - 부분적으로만 답변 가능하거나, 핵심 정보(수치, 조건, 절차 등)가 빠져있으면 불충분
3. **정보 최신성/정확성**: FAQ 내용이 질문에 대한 **정확한 답**을 포함하고 있는가?
   - FAQ가 일반론만 다루고 구체적 기준/수치가 없으면 불충분
4. **추측 불필요성**: FAQ 내용을 **해석하거나 추론하지 않고** 그대로 전달해도 충분한 답변이 되는가?
   - FAQ에 없는 내용을 보충해야 하거나, 여러 FAQ를 조합해 추론해야 하면 불충분
5. **단일 FAQ 대응**: 1~2개의 FAQ 문서로 답변이 완결되는가?
   - 3개 이상의 FAQ를 복합적으로 조합해야 하면 운영기준 검색이 더 적합

### 종합 판단
- 위 5가지 중 **4가지 이상 충족** → sufficient: true
- **3가지 이하 충족** → sufficient: false
- 애매하면 **false** (운영기준 검색으로 폴백하는 것이 더 안전)

### 출력 형식 (JSON만 출력, 다른 텍스트 없이)
{{"sufficient": true또는false, "confidence": 1에서5사이정수(충족항목수), "reason": "판단 근거를 한 문장으로 설명"}}"""
