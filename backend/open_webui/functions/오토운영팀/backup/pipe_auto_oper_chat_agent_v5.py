...
#   사용자 질문
#       ↓
#   [1] _rule_react_agent() : 웹규정집 ReAct 검색 (LLM 1~2회, 하이브리드 검색)
#       ↓
...
class Pipe:
    class Valves(BaseModel):
        ...
        RULE_FILTER_DOCS: str = "E09003,E09012,E09024,B03004,B03001"
        # ── 웹규정집 상품 키워드 매핑 (유사 규정 구분용) ──
        # JSON 형식: {"키워드": "규정코드", ...}
        RULE_PRODUCT_KEYWORDS: str = '{"중고승용":"E09003","중고차승용":"E09003","승용차":"E09003","승용":"E09003","중고차":"E09003","재고금융":"E09012","운영자금":"E09012","중고리스":"E09024","오토리스":"E09024","중고오토리스":"E09024","복지":"B03004","인사":"B03001"}'
    ...
    def __init__(self):
        ...
        self._rule_bm25_index = None
        self._rule_bm25_docs = []
            ...
            if self.valves.ENABLE_RULE_HYBRID_SEARCH:
                try:
                    ...
                    self._rule_bm25_docs = [
                        ...
                        if meta.get("chpater_merge_yn") != "Y"  # 장 merge 문서 제외 (너무 큼)
                    ]

                    if self._rule_bm25_docs:
                        tokenized_corpus = [d.page_content.split() for d in self._rule_bm25_docs]
                        self._rule_bm25_index = BM25Okapi(tokenized_corpus)
                        print(f"웹규정집 BM25 인덱스 구축 완료: {len(self._rule_bm25_docs)}건 문서")
                except Exception as e:
                    print(f"웹규정집 BM25 인덱스 구축 실패 (벡터 검색만 사용): {e}")

        print("통합 에이전트 Pipe 등록 완료! (v5 - 웹규정집 + FAQ + 운영기준 3계층 폴백)")
    ...
    async def _rule_react_agent(self, state: dict) -> dict:
        ...
        # 상품 키워드 감지 → 메타데이터 필터링에 사용
        detected_product = self._detect_rule_product(state["user_message"])
        state["detected_product"] = detected_product
        if detected_product:
            print(f"[규정집] 상품 감지: {detected_product}")
        ...
        for i in range(self.valves.RULE_REACT_MAX_ITERATIONS):
            ...
            if decision["action"] == "SUFFICIENT":
                ...
                raw_docs = self._rule_execute_tool(query, product_filter=detected_product)
    ...
    # _rule_execute_tool() - 웹규정집 하이브리드 검색 (벡터 + BM25 + RRF)
    # =================================================================
    def _rule_execute_tool(self, query: str, product_filter: Optional[str] = None) -> list:
        k = self.valves.RULE_EMBEDDING_K

        # 상품 감지 시 ChromaDB 메타데이터 필터 구성
        chroma_filter = None
        if product_filter:
            chroma_filter = {"title": {"$contains": product_filter}}

        use_hybrid = (
            self.valves.ENABLE_RULE_HYBRID_SEARCH
            and self._rule_bm25_index is not None
            and len(self._rule_bm25_docs) > 0
        )
        ...
        if not use_hybrid:
            ...
            return self.chroma_db_rule.similarity_search(query=query, **search_kwargs)

        search_k = k * 2

        vector_kwargs = {"k": search_k}
        if chroma_filter:
            vector_kwargs["filter"] = chroma_filter
        vector_results = self.chroma_db_rule.similarity_search(query=query, **vector_kwargs)

        tokenized_query = query.split()
        scores = self._rule_bm25_index.get_scores(tokenized_query)
        scored_indices = [
        ...
        if product_filter:
            bm25_results = [
                self._rule_bm25_docs[idx] for idx, _ in scored_indices
                if product_filter in self._rule_bm25_docs[idx].metadata.get("title", "")
            ][:search_k]
        else:
            bm25_results = [self._rule_bm25_docs[idx] for idx, _ in scored_indices[:search_k]]
