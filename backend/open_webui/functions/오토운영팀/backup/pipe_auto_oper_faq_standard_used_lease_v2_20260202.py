...
class Pipe:
    ...
    def __init__(self):
        ...
        self.retriever1 = stand_chroma_db_text.as_retriever(search_kwargs={"filter": {"subtask": "(중고리스)"}, "k": embedding_k})
    ...
    async def pipe(
        ...
        try:
            ...
            await send_status(status_message=f"분류 결과 : 중고리스 > {classified}", done = True)
            
            
            if classified == "재질의":
                generation = await self.common_generate(user_message = user_message, history=history)
                return generation
            elif classified == '중고리스':
                documents = self.retriever1.invoke(user_message)
                filtered_docs, next_answer = await self.grade_documents(documents=documents, user_message=user_message, FLAG="중고리스 > 중고리스")
    ...
    async def grade_documents(self, user_message, documents, FLAG="FAQ"):
        ...
        참고로, 우리는 4가지의 업무영역이 있고 라우터로 처리되고 있어.
    ...
    async def common_generate(self, user_message, history):
        ...
        # 테스트 데이터 생성
        system = """
        지금 너에게 물어보는 경우에는 '중고리스' 업무 분류에 벗어난 질문을 한 상태거나, '중고리스'문의를 너무 광범위하게 질문한 상태야
        재질문으로 사용자에게 다시 입력을 유도해.
        보통은 "내용에서 벗어난 질문입니다. 중고리스 업무 중 취급·운용·심사·계약·서류·보험·세금·잔가·금리 기준·절차 중 문의해주세요." 정도로 답하고
        가능하면 사용자 질문이나 그간 대화내용을 참고해서 더 나은 질문에 대해서 답변해줘. 
        예를들어 "운영기준 알려줘"와 같은 너무 넓은 범위 질문이 들어오면,
        추천질문으로 
        운용리스와 금융리스의 가장 큰 차이점은 무엇인가요?
        수입차 리스에서 차량이 “I-P군” 중 어느 그룹에 속하는지 확인하려면 어떤 자료를 보면 되나요?
        예를 들어 Mercedes-Benz GLC AMG를 48개월 리스로 계약하고 싶다면, 어느 그룹에 속하고 보장잔가는 어떻게 계산하나요?
        A-H군과 I-P군의 잔가율 차이를 알려주세요. 특히 36개월 기준으로 각각 어느 정도인가요?
        명의이전 전·후 송금 시 반드시 제출해야 하는 서류 목록과 순서는 어떻게 되나요?
        와 같은 추천질문을 제공함으로써, 사용자가 업무 영역으로 넘어가도록 추천질문을 제공해줘.
        너는 반드시 "한국어"로 답변해.               
        """
    ...
    async def generate(self, documents, user_message, history, classified, last_classified):
        ...
        너가 대답하는 경우는 오토운영팀 중고리스 대출 상품에 대해 상세 문의 하는 경우야
        사용자에게 주어진 질문에 대해서 참고자료를 활용해서 참고 원본을 살리면서 질문에 적절히 답변해
        
        [중고리스 업무정의]
        중고리스를(운용·금융) 상품의 전반적인 취급·운용·심사·계약·서류·보험·세금·잔가·금리 기준·절차를 정의하고,
        실제 운용 시 필요한 서류·검증·잔가·잔가율·차량 제한사항 등을 구체화한다. 
        특히 수입차 리스에 대해 '잔가표 (그룹 I-P)' 와 '잔가율표 (그룹 I-P)' 를 신설·보완하여, 차종·모델 별 잔가·보장잔가 산정을 명확히 한다.
    ...
    async def etc_generate(self, documents, user_message, history):
        ...
        1. 제공되는 참고자료를 참고하여 적합한 답변을 생성하고 DocumentID 같은 IT 단어는 사용하지마
        2. 사용자 질문이 너무 광범위할 경우, 구체적으로 알려달라고 요청해.
        3. 사용자가 질문에 특정 업무를 명시하지 않는 이상, 기존에 사용하던 업무라고 인식해. 
        4. 분류가 모호한 경우 가능하면 '재질의'로 처리해서 크리티컬한 실수가 발생하지 않도록 해
    ...
    async def requery_and_classify_task(self, question):
        ...
        [분류 기준]
        분류 기준은 '중고리스', '재질의' 이며 1개 업무와 1개 업무를 벗어난 '재질의'로 분류하시오.
        사용자는 특정 업무가 분류된 이후로 업무에 관련된 상세 질문을 추가할 수 있습니다.(취급·운용·심사·계약·서류·보험·세금·잔가·금리 기준·절차 등) 이점 참고하세요
        
        <중고리스>
        정의 : 중고리스를(운용·금융) 상품의 전반적인 취급·운용·심사·계약·서류·보험·세금·잔가·금리 기준·절차를 정의하고, 실제 운용 시 필요한 서류·검증·잔가·잔가율·차량 제한사항 등을 구체화한다. 특히 수입차 리스에 대해 '잔가표 (그룹 I-P)' 와 '잔가율표 (그룹 I-P)' 를 신설·보완하여, 차종·모델 별 잔가·보장잔가 산정을 명확히 한다.
        관련 질의 : 
        운용리스와 금융리스의 가장 큰 차이점은 무엇인가요?
        수입차 리스에서 차량이 “I-P군” 중 어느 그룹에 속하는지 확인하려면 어떤 자료를 보면 되나요?
        예를 들어 Mercedes-Benz GLC AMG를 48개월 리스로 계약하고 싶다면, 어느 그룹에 속하고 보장잔가는 어떻게 계산하나요?
        ...
        <재질의>
        정의 : '중고리스' 외 상품 문의(업무관련 공통 질의 포함) 또는 상품 중 모호한 질의인 경우 재질의 실시
        관련 질의 : 중고리스 상품에 대해서 알려줘 또는 운영기준 알려줘
        예상 답변 : '중고리스'에 대해서 문의해주세요.
            
        
        다음 JSON 스키마를 정확히 준수하여 한국어로만 응답하시오.
        {{
        "classifed_task": "중고리스|재질의"
        }}
