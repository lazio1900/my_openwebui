...
# 아키텍처 요약
# ──────────────────────────────────────────
# [흐름] FAQ ReAct 우선 → 운영기준 ReAct 폴백
#
#   사용자 질문
#       ↓
#   [1] _faq_react_agent()  : FAQ ReAct 검색 (LLM 1~2회, 하이브리드 검색)
#       ↓
#       ├─ FAQ 충분 → _generate(faq) → 종료    ← classify 스킵
#       ↓
#   [2] _classify()         : 도메인/서브태스크 분류 (LLM 1회)
#       ↓
#       ├─ 재질의 → _requery() → 종료
#       ↓
#   [3] _react_agent()      : 운영기준 ReAct 검색 (LLM 1~2회)
#       ↓
#   [4] _generate(standard) → 종료
#
# [핵심 메서드]
...
class Pipe:
    ...
    def __init__(self):
        ...
        print("통합 에이전트 Pipe 등록 완료! (v4 - FAQ ReAct + 전체 이미지 노출)")

    # =================================================================
    # pipe() - 메인 엔트리포인트
    # ---------------------------------------------------------------
    # 흐름:
    #   1. _faq_react_agent() : FAQ ReAct 검색 (LLM 1~2회)
    ...
    async def _generate(self, state: dict):
        ...
        followup_task = None
        grounding_task = None
        if self.valves.ENABLE_FOLLOWUPS and answer_source != "faq":
